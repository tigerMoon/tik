...
export class ContextRenderer {
    ...
    renderEnvironmentPack(environment, agent) {
    ...
    renderRepo(repo, agent) {
        // Only planner and coder benefit from repo structure
        if (agent === 'reviewer')
            return null;
        const repoWithCandidates = repo;
    ...
    renderSearchGuidance(repo, agent) {
        const repoWithCandidates = repo;
    ...
    renderOperatorGuidance(comments) {
        const parts = [
            '# Operator Guidance',
...
function formatOperatorCommentDate(value) {
...
//# sourceMappingURL=context-renderer.js.map