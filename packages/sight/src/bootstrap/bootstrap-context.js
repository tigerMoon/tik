...
export class BootstrapContextBuilder {
    ...
    async getGitStatus(projectPath) {
        try {
            const { stdout } = await execFileAsync('git', ['status', '--short', '--branch'], { cwd: projectPath, timeout: 5_000 });
    ...
    async getGitDiff(projectPath) {
        try {
            // Staged + unstaged
            const { stdout: staged } = await execFileAsync('git', ['diff', '--cached', '--stat'], { cwd: projectPath, timeout: 5_000 });
            const { stdout: unstaged } = await execFileAsync('git', ['diff', '--stat'], { cwd: projectPath, timeout: 5_000 });
     ...
     */
    async discoverInstructionFiles(projectPath) {
        const directories = [];
...
//# sourceMappingURL=bootstrap-context.js.map