...
import { RepoCandidateFinder } from './repo-candidate-finder.js';
...
export class ContextEngine {
    ...
    compactor;
    candidateFinder;
    environmentPackLoader;
    constructor(projectPath, eventBus) {
    ...
    async buildContext(taskId, iteration, options) {
        const startTime = now();
        ...
        context.environment = await this.environmentPackLoader.load(options?.environmentPackId, options?.environmentPackSelection);
        return context;
    }
    /** Build session-aware context returning RuntimeContextEnvelope (Phase 2.8) */
    async buildFromSession(task, session, options) {
        const startTime = now();
        ...
        const conversation = {
            recentMessages: compacted.messages.slice(-5).map((m) => ({
                role: m.role,
                content: m.content,
                name: m.name,
            })),
        ...
        if (metaFrag) {
            try {
                ...
                if (projectFile === 'package.json') {
                    ...
                    if (artifactMatch)
                        modules.push({ name: artifactMatch[1], path: '.', type: 'application', dependencies: [] });
                }
            }
            catch { }
        }
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    for (const f of parsed.slice(-10)) {
                        failures.push({ type: (f.type || 'build'), message: f.message || '', timestamp: f.timestamp || 0 });
                    }
                }
            }
            catch { }
    ...
    extractMemoryContext(fragments) {
        if (fragments.length === 0)
            return undefined;
        return {
            blocks: [],
