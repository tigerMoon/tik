...
const tempDirs = [];
async function createWorkspaceResolution() {
...
describe('WorkspaceOrchestrator', () => {
    it('bootstraps into PARALLEL_CLARIFY and advances phases as projects complete', async () => {
        ...
        await orchestrator.markClarifyResult(resolution.workspace.rootPath, 'service-a', path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-a/clarify-1.md'), 'clarify a', 'task-clarify-a');
        ...
        await orchestrator.markClarifyResult(resolution.workspace.rootPath, 'service-b', path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-b/clarify-1.md'), 'clarify b', 'task-clarify-b');
    ...
    it('records feedback and moves to FEEDBACK_ITERATION with nextPhase metadata', async () => {
        ...
        const snapshot = await orchestrator.recordFeedback(resolution.workspace.rootPath, 'plan 仍是模板，需要重跑 plan', ['service-a', 'service-b'], 'PARALLEL_CLARIFY');
    ...
    it('preserves plan task ids and raises workspace feedback when plan is blocked', async () => {
        ...
        const serviceA = resolution.workspace.projects[0];
        await orchestrator.markClarifyResult(resolution.workspace.rootPath, 'service-a', path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-a/clarify-1.md'), 'clarify a', 'task-clarify-a');
        await orchestrator.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a');
        const snapshot = await orchestrator.markProjectBlocked(resolution.workspace.rootPath, 'service-a', 'PARALLEL_PLAN', 'Generated plan still looks like a template skeleton.', 'task-plan-a');
    ...
    it('captures clarify blockers as pending decisions before specification begins', async () => {
        ...
        const clarificationPath = path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-a/clarify-1.md');
        const snapshot = await orchestrator.markClarifyBlocked(resolution.workspace.rootPath, 'service-a', clarificationPath, 'Clarification category: scope\nClarification method: deep-interview', 'task-clarify-a');
        const project = snapshot.state?.projects?.find((item) => item.projectName === 'service-a');
    ...
    it('preserves a clarifier-provided decision instead of re-synthesizing one from the blocker summary', async () => {
        ...
        const snapshot = await orchestrator.markClarifyBlocked(resolution.workspace.rootPath, 'service-a', path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-a/clarify-1.md'), 'Clarification category: generic\nClarification method: deep-interview', 'task-clarify-a', 'awaiting_decision', suppliedDecision);
        expect(snapshot.state?.decisions?.[0]).toMatchObject({
            id: 'decision-clarify-1',
            title: 'Clarify service-a before specify',
            confidence: 'low',
    ...
    it('creates structured approach-choice decisions for ambiguous artifact blockers and resolves them back into feedback', async () => {
        ...
        const snapshot = await orchestrator.markProjectBlocked(resolution.workspace.rootPath, 'service-a', 'PARALLEL_SPECIFY', 'Multiple feature specs found; unable to choose automatically: /tmp/specs/feature-a/spec.md, /tmp/specs/feature-b/spec.md', 'task-spec-a');
        ...
        const resolved = await orchestrator.resolveDecision(resolution.workspace.rootPath, {
    ...
    it('marks clarify decisions as resolved in project state after decide', async () => {
        ...
        const blocked = await orchestrator.markClarifyBlocked(resolution.workspace.rootPath, 'service-a', path.join(resolution.workspace.rootPath, '.workspace/clarifications/service-a/clarify-1.md'), 'Clarification category: generic\nClarification method: deep-interview', 'task-clarify-a');
    ...
    it('persists executionMode on completed document phases', async () => {
        ...
        const serviceA = resolution.workspace.projects[0];
        const snapshot = await orchestrator.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a', 'fallback');
        const project = snapshot.state?.projects?.find((item) => item.projectName === 'service-a');
    ...
    it('persists executionMode on completed ace phases', async () => {
        ...
        const serviceA = resolution.workspace.projects[0];
        await orchestrator.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a', 'native');
        await orchestrator.markPlanResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/plan.md'), 'plan a', 'task-plan-a', 'native');
        const snapshot = await orchestrator.markAceResult(resolution.workspace.rootPath, 'service-a', 'task-ace-a', 'completed', 'ACE task completed', 'native');
        const project = snapshot.state?.projects?.find((item) => item.projectName === 'service-a');
    ...
    it('updates workflow policy after bootstrap for later workspace iterations', async () => {
        ...
        const snapshot = await orchestrator.updateWorkflowPolicy(resolution.workspace.rootPath, { profile: 'fast-feedback' });
    ...
    it('preserves ace task ids and raises workspace feedback when ace fails', async () => {
        ...
        const serviceA = resolution.workspace.projects[0];
        await orchestrator.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a');
        await orchestrator.markPlanResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/plan.md'), 'plan a', 'task-plan-a');
        const snapshot = await orchestrator.markAceResult(resolution.workspace.rootPath, 'service-a', 'task-ace-a', 'failed', 'ACE task failed');
        const project = snapshot.state?.projects?.find((item) => item.projectName === 'service-a');
    ...
    it('classifies timed-out ACE blockers as execution failures with actionable retry guidance', async () => {
        ...
        await orchestrator.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a');
        await orchestrator.markPlanResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/plan.md'), 'plan a', 'task-plan-a');
        const snapshot = await orchestrator.markProjectBlocked(resolution.workspace.rootPath, 'service-a', 'PARALLEL_ACE', 'ACE execution failed: Codex exec fallback did not finish within 90s.', 'task-ace-a');
        const project = snapshot.state?.projects?.find((item) => item.projectName === 'service-a');
    ...
    it('uses a cross-instance lock so concurrent state mutations do not lose updates', async () => {
        ...
        const serviceB = resolution.workspace.projects[1];
        await Promise.all([
            orchestratorA.markSpecifyResult(resolution.workspace.rootPath, 'service-a', path.join(serviceA.path, '.specify/specs/spec.md'), 'spec a', 'task-spec-a'),
            orchestratorB.markSpecifyResult(resolution.workspace.rootPath, 'service-b', path.join(serviceB.path, '.specify/specs/spec.md'), 'spec b', 'task-spec-b'),
        ]);
...
//# sourceMappingURL=workspace-orchestrator.test.js.map