...
const tempDirs = [];
const servers = [];
const TEST_ENVIRONMENT_SNAPSHOT = {
    id: 'test-engineering',
...
class CompletingRuntimeRunner {
    ...
    async start(input) {
        ...
        const completion = Promise.resolve({ status: 'completed' }).then((result) => {
...
async function writeWorkflowV2Fixture(root) {
...
describe('workbench API routes', () => {
    it('requires bearer auth for mutating routes when an API token is configured', async () => {
        ...
        const server = await createServer(mockKernel, { port: 0, host: '127.0.0.1' }, { workspaceRoot: root, apiToken: 'test-token' });
        servers.push(server);
    ...
    it('refuses non-localhost binds without an API token', async () => {
        ...
        await expect(createServer(mockKernel, { port: 0, host: '0.0.0.0' }, { workspaceRoot: root })).rejects.toThrow('API token is required');
    });
    ...
    it('defaults CORS to the dashboard origin instead of wildcard', async () => {
        ...
        const server = await createServer(mockKernel, { port: 0, host: '127.0.0.1' }, { workspaceRoot: root });
        servers.push(server);
    ...
    it('exposes tracker task CRUD and mutation endpoints under /api/v1', async () => {
        ...
        expect(listResponse.statusCode).toBe(200);
        expect(listResponse.json().tasks.map((task) => task.id)).toContain(taskId);
    });
    ...
    it('routes and manually runs a workflow v2 task through the workbench API', async () => {
        ...
        const server = await createServer(mockKernel, { port: 0, host: '127.0.0.1' }, { workspaceRoot: root, runtimeRunners: { 'claude-code': runtimeRunner } });
        servers.push(server);
        const preview = await server.inject({
            method: 'GET',
            url: `/api/v1/tasks/${task.id}/routing-preview`,
    ...
    it('reports tracker watching only when persisted watch state has a visible local process', async () => {
        ...
        await fs.writeFile(path.join(root, '.tik', 'tracker-daemon', 'state.json'), JSON.stringify({
            watching: true,
            retries: {},
            recent: [
                {
        ...
        expect(body.listeners.find((listener) => listener.id === 'tracker-watch')?.status)
            .toBe('expected');
        const dashboard = body.listeners.find((listener) => listener.id === 'dashboard');
    ...
    it('updates task-level skill and knowledge configuration through the workbench API', async () => {
        ...
        const mockKernel = {
            ...
            taskManager: {
                create: () => ({ id: 'unused' }),
                get: (taskId) => taskState.get(taskId),
    ...
    it('updates the task brief through the workbench API and syncs the kernel task description', async () => {
        ...
        expect(timelineResponse.statusCode).toBe(200);
        expect(timelineResponse.json().timeline.some((item) => (item.actor === 'user' && item.body.includes('Adjusted task brief')))).toBe(true);
    });
    ...
    it('archives inactive tasks and hides them from active workflows without deleting history', async () => {
        ...
        expect(taskListResponse.json().tasks.find((item) => item.id === task.id)?.status).toBe('archived');
    });
    ...
    it('serves legacy previewable artifact files only when explicitly enabled', async () => {
        ...
        const server = await createServer(mockKernel, { port: 0, host: '127.0.0.1' }, {
            workspaceRoot: root,
    ...
    it('exposes first-class artifact registry routes and artifact-id preview', async () => {
        ...
        expect(listResponse.statusCode).toBe(200);
        expect(listResponse.json().artifacts.map((item) => item.id)).toEqual([artifact.id]);
        ...
        expect(versionsResponse.json().versions.map((version) => version.version)).toEqual([1, 2]);
        const previewResponse = await server.inject({
            method: 'GET',
    ...
    it('generates task-review artifacts as versions with provenance and renders markdown previews as HTML', async () => {
        ...
        expect(taskArtifactsResponse.json().artifacts
            .filter((artifact) => artifact.producedBy?.template === 'task-review')
            .map((artifact) => artifact.id)).toEqual([firstArtifact.id]);
        const previewResponse = await server.inject({
            method: 'GET',
...
async function waitForFile(filePath, timeoutMs = 200) {
...
//# sourceMappingURL=server-workbench-api.test.js.map