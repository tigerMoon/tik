...
describe('WorkflowSkillExecutorRegistry', () => {
    it('registers and executes contract-bound executors', async () => {
        ...
        const executor = vi.fn(async (_request) => ({
            summary: 'done',
    ...
    it('creates default workspace executors that use structured demand inputs', async () => {
        ...
        const completion = vi.fn(async (_projectPath, prompt) => {
        ...
        expect(completion).toHaveBeenCalledWith(projectPath, expect.stringContaining('给 service-a 增加缓存'), expect.any(Object));
        expect(completion).toHaveBeenCalledWith(projectPath, expect.stringContaining('Skill Runtime Binding:'), expect.any(Object));
...
//# sourceMappingURL=workflow-skill-executor.test.js.map