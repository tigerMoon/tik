...
class MemoryTaskImporter {
    ...
    async listCandidateTasks() {
        if (this.failListError)
            throw this.failListError;
...
class RecordingLauncher {
    ...
    async markRuntimeRunFinished(taskId, input) {
        ...
        await this.workbench.finishAttempt?.(taskId, input.attemptNumber, input.completion.status === 'completed' ? 'completed' : 'failed', input.completion.error);
...
describe('tracker-daemon Symphony spec behavior', () => {
    ...
    it('fails prompt rendering on unknown workflow variables', async () => {
        ...
        const workflow = await loadTrackerWorkflow(root);
        expect(() => workflow.renderPrompt(task('task-1', 'TIK-1'))).toThrow('Unknown workflow template variable: issue.missing_field');
    });
    it('keeps issue template aliases for workflow v2 files', async () => {
        ...
        const workflow = await loadTrackerWorkflow(root);
        expect(workflow.renderPrompt(task('task-1', 'TIK-1'))).toContain('Implement TIK-1 from https://linear.local/TIK-1.');
    });
    ...
    it('runs dispatch hooks for selected tasks concurrently within the agent limit', async () => {
        ...
        const launcher = new RecordingLauncher();
        let hookStarts = 0;
        let releaseFirstHook;
        const firstHookGate = new Promise((resolve) => {
...
//# sourceMappingURL=tracker-daemon-spec.test.js.map