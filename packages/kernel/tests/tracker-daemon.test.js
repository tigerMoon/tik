...
function task(id, shortIdentifier, stateKind = 'active') {
    return {
        id,
        shortIdentifier,
...
async function waitForWorkbenchTask(workbench, taskId, predicate) {
...
describe('TrackerDaemon', () => {
    ...
    it('finishes the attempt and fails the workbench task when background kernel execution rejects', async () => {
        ...
        const workbenchTask = await waitForWorkbenchTask(workbench, 'tracked-task-id', (candidate) => (candidate?.status === 'failed'
    ...
    it('records Claude review output as a task comment and routes blocking reviews to Codex fix', async () => {
        ...
        await daemon.tick();
        const updated = await waitForWorkbenchTask(workbench, 'task-review-output', (candidate) => (candidate?.status === 'todo'
    ...
    it('routes clean Claude review output to human review without triggering Codex fix', async () => {
        ...
        await daemon.tick();
        const updated = await waitForWorkbenchTask(workbench, 'task-review-clean', (candidate) => (candidate?.status === 'in_review'
    ...
    it('returns Codex fix completions to Claude review for the next loop round', async () => {
        ...
        await daemon.tick();
        const updated = await waitForWorkbenchTask(workbench, 'task-fix-complete', (candidate) => (candidate?.status === 'todo'
    ...
    it('generates a run proof and review artifact when a workflow v2 runtime completes', async () => {
        ...
        const updated = await waitForWorkbenchTask(workbench, 'task-proof', async (candidate) => (candidate?.status === 'needs_review'
    ...
    it('renders a Tik-generated fix prompt for Codex agent-loop phases', async () => {
        ...
        await waitForWorkbenchTask(workbench, 'task-fix-prompt', (candidate) => (candidate?.status === 'todo'
            && candidate.labels?.includes('needs-claude-review')
            && candidate.agentLoop?.kind === 'claude_review'));
    });
    ...
    it('forwards only human comments (most recent first, capped at 5) into the kernel task as recentComments', async () => {
        ...
        const recentComments = captureKernelTaskInputs[0].recentComments;
    ...
    it('passes a compact task context snapshot when relaunching an existing workbench task', async () => {
        ...
        expect(captureKernelTaskInputs).toHaveLength(1);
        const snapshot = captureKernelTaskInputs[0].taskContextSnapshot;
...
//# sourceMappingURL=tracker-daemon.test.js.map