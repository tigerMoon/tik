...
describe('codex evaluator and Claude questioner workflow', () => {
    ...
    it('ignores non-record artifact json files in the Questioner storage directory', async () => {
        ...
        await fs.writeFile(path.join(noisyDir, 'q-artifact-copy.json'), JSON.stringify({
            id: 'q-artifact-copy',
            source: 'claude-plugin',
            intent: 'question_evaluation',
    ...
    it('records complete-subtask through a single guarded action endpoint', async () => {
        ...
        expect(bundle.json().decisions.map((decision) => decision.id)).toEqual(['dec-action-complete-subtask']);
...
async function createV1Workflow(server, workflowId, root) {
...
async function putSingleSubtaskGraph(server, workflowId) {
    const response = await server.inject({
        method: 'PUT',
...
async function createAcceptedContract(server, workflowId, options = {}) {
...
async function moveSubtaskToQuestioningEvidence(server, workflowId, subtaskId, evidenceId = 'ev-impl') {
...
async function recordPassingEvaluation(server, workflowId, evaluationId) {
    const response = await server.inject({
        method: 'POST',
...
async function createInvocationRecord(server, workflowId, input) {
    return server.inject({
        method: 'POST',
...
async function moveSubtaskToDone(server, workflowId, subtaskId) {
...
//# sourceMappingURL=multi-agent-codex-evaluator-questioner.test.js.map