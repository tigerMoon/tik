...
describe('CodexHarnessAdapter', () => {
    it('maps app-server notifications into text and provider events', async () => {
        ...
        const onNotification = vi.spyOn(CodexAppServerClient.prototype, 'onNotification').mockImplementation((method, listener) => {
        ...
        const request = vi.spyOn(CodexAppServerClient.prototype, 'request').mockImplementation(async (method) => {
        ...
        expect(request).toHaveBeenCalledWith('thread/start', expect.objectContaining({ cwd: '/tmp/project' }), expect.anything());
        expect(request).toHaveBeenCalledWith('turn/start', expect.objectContaining({ threadId: 'thread-1' }), expect.anything());
...
//# sourceMappingURL=codex-harness-adapter.test.js.map