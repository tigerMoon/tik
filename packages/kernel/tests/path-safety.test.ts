...
describe('safeResolveWorkspacePath', () => {
  ...
  it('allows directories only when requested', async () => {
    ...
    await expect(safeResolveWorkspacePath(root, 'src', { allowDirectory: true })).resolves.toBe(
  ...
  it('preserves nested missing paths inside the workspace', async () => {
    ...
    await expect(safeResolveWorkspacePath(root, 'new/deep/file.ts')).resolves.toBe(
      path.join(root, 'new', 'deep', 'file.ts'),
    );
  });
