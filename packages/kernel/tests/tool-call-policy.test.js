...
import { classifyTaskIntent, assistantSuggestsImplementationComplete, assistantSuggestsNoCodeChangeNeeded, dedupeToolCalls, enoughEvidenceToConclude, getToolCallSignature, hasMeaningfulPendingWork, hasSuccessfulValidationAction, isVerificationProbeBatch, isRedundantReadBatch, normalizeToolCall, sessionMemorySuggestsImplementation, shouldMarkTaskCompleted, shouldShiftFromExplorationToImplementation, } from '../src/agent/tool-call-policy.js';
describe('tool-call policy', () => {
    ...
    it('detects when assistant text already concludes implementation exists', () => {
        expect(assistantSuggestsImplementationComplete('我已经查看了代码，发现缓存功能已经实现。让我检查一下是否有遗漏的部分：')).toBe(true);
        expect(assistantSuggestsImplementationComplete('The cache layer has already been implemented; I will check if anything is missing.')).toBe(true);
    });
    it('detects verification probe batches', () => {
        expect(isVerificationProbeBatch([
    ...
    it('does not mark implementation tasks completed after read-only analysis', () => {
        expect(shouldMarkTaskCompleted('我想针对票务业务的查询接口做缓存，one-api目录', {
            goal: 'add cache',
            keyFiles: ['/repo/AccountCacheManager.java', '/repo/CatalogQueryServiceImpl.java'],
    ...
    it('does not mark implementation tasks completed after writing code without verification', () => {
        expect(shouldMarkTaskCompleted('实现票务查询接口缓存', {
            goal: 'add cache',
            keyFiles: ['/repo/AccountCacheManager.java', '/repo/CatalogQueryServiceImpl.java'],
...
//# sourceMappingURL=tool-call-policy.test.js.map