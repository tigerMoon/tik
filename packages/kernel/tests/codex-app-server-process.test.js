...
describe('CodexAppServerProcess', () => {
    it('spawns codex app-server stdio and parses json lines', async () => {
        ...
        const process = new CodexAppServerProcess({ cwd: '/tmp/project' }, spawnFactory);
        const messages = [];
        const stderr = [];
        process.onMessage((message) => messages.push(message));
        process.onStderr((chunk) => stderr.push(chunk));
        await process.start();
        expect(spawnFactory).toHaveBeenCalledWith('codex', ['app-server', '--listen', 'stdio://'], expect.objectContaining({ cwd: '/tmp/project', stdio: 'pipe' }));
...
//# sourceMappingURL=codex-app-server-process.test.js.map