...
describe('multi-agent v1 core runtime API', () => {
    it('stores loop contracts on workflow policy through PATCH without replacing existing policy', async () => {
        ...
        const stored = JSON.parse(await fs.readFile(path.join(root, '.tik', 'multi-agent', 'workflows', 'wf-loop-contract', 'workflow.json'), 'utf-8'));
    ...
    it('rejects stale decision commits with If-Match and returns the latest workflow state', async () => {
        ...
        expect(read.json().decisions.map((decision) => decision.id)).toEqual(['dec-first']);
    });
    ...
    it('persists context snapshots with rendered markdown, etags, and local mirrors', async () => {
        ...
        const mirror = await fs.readFile(path.join(root, '.tik', 'multi-agent', 'workflows', 'wf-snapshot', 'context', 'main.snapshot.md'), 'utf-8');
    ...
    it('applies target snapshot defaults, truncates rendered markdown, and de-duplicates unchanged snapshot events', async () => {
        ...
        const snapshotEvents = timeline.json().events
            .filter((event) => event.type === 'context_snapshot.recorded');
        expect(snapshotEvents.map((event) => event.payload.target)).toEqual(['builder', 'evaluator']);
    });
    it('marks stalled Codex invocations failed and blocks the workflow with audit timeline evidence', async () => {
        ...
        expect(timeline.json().events.map((event) => event.type)).toContain('invocation.stalled');
    });
    it('records human overrides with guard rejection audit context and can resume blocked workflows', async () => {
        ...
        expect(timeline.json().events.map((event) => event.type)).toContain('workflow.human_override');
    });
...
async function createWorkflow(server, payload) {
    const created = await server.inject({
        method: 'POST',
...
async function putTaskGraph(server, workflowId, subtasks) {
...
async function blockWorkflowWithStalledInvocation(server, workflowId, subtaskId) {
    const created = await server.inject({
        method: 'POST',
...
async function writeInvocationJsonl(root, workflowId, invocation) {
    await fs.writeFile(path.join(root, '.tik', 'multi-agent', 'workflows', workflowId, 'invocations.jsonl'), `${JSON.stringify(invocation)}\n`, 'utf-8');
}
function buildDecision(workflowId, patch) {
    return {
        workflowId,
...
function buildLoopContract(workflowId, input) {
    return {
...
function buildSnapshot(workflowId, patch) {
    return {
        workflowId,
        headSha: 'head-1',
...
//# sourceMappingURL=multi-agent-v1-runtime-api.test.js.map