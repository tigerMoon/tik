...
describe('multi-agent coordination API', () => {
    it('persists workflow state, task graph, decisions, evidence, and timeline without choosing policy actions', async () => {
        ...
        expect(timeline.json().events.map((event) => event.type)).toEqual([
            'workflow.created',
            'task_graph.created',
            'evidence.recorded',
            'evidence.recorded',
        ...
        const storedWorkflow = JSON.parse(await fs.readFile(path.join(root, '.tik', 'multi-agent', 'workflows', 'wf-auth', 'workflow.json'), 'utf-8'));
...
function buildTaskGraph(workflowId, version, subtasks) {
    return {
        workflowId,
        version,
        createdBy: 'claude-code',
...
async function moveSubtaskToDone(server, workflowId, subtaskId, headSha) {
    await server.inject({
        method: 'POST',
...
//# sourceMappingURL=multi-agent-coordination-api.test.js.map