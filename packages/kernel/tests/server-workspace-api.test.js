...
describe('workspace API server routes', () => {
    it('serves workspace status, memory, and decisions endpoints for non-CLI consumers', async () => {
        ...
        const server = await createServer(mockKernel, { port: 0, host: '127.0.0.1' }, { workspaceRoot: resolution.workspace.rootPath });
        servers.push(server);
        ...
        await orchestrator.markProjectBlocked(resolution.workspace.rootPath, 'service-a', 'PARALLEL_SPECIFY', 'Multiple feature specs found; unable to choose automatically: /tmp/specs/feature-a/spec.md, /tmp/specs/feature-b/spec.md', 'task-spec-1');
    ...
    it('serves and mutates managed worktree lanes through workspace API routes', async () => {
        ...
        expect(createResponse.json().worktrees.entries.some((entry) => entry.laneId === 'feature-a')).toBe(true);
        ...
        expect(listResponse.json().worktrees.entries.some((entry) => entry.laneId === 'feature-a')).toBe(true);
        ...
        expect(removeResponse.json().state.projects[0].worktreeLanes.some((lane) => lane.laneId === 'feature-a' && lane.status === 'removed')).toBe(true);
    });
});
//# sourceMappingURL=server-workspace-api.test.js.map