...
export class ToolScheduler {
    ...
    activeExec = null;
    awaitToolApproval;
    constructor(registry, eventBus, options = {}) {
    ...
    async execute(toolName, input, context) {
        ...
        const eventOutput = outputStr && outputStr.length > EVENT_OUTPUT_LIMIT
            ...
            : result.output;
        this.emitEvent(result.success ? EventType.TOOL_RESULT : EventType.TOOL_ERROR, context.taskId, {
            toolName,
            output: eventOutput,
    ...
    async executeBatch(actions, context) {
    ...
    // ─── Private Methods ──────────────────────────────────────
    async executeReadyActions(ready, actions, results, completed, context) {
        let cursor = 0;
        ...
        while (cursor < ready.length) {
            ...
            if (readParallelAllowed && tool?.type === 'read') {
                ...
                while (cursor < ready.length) {
                    ...
                    if (readTool?.type !== 'read')
                        break;
                    readBatch.push(readIndex);
                    cursor += 1;
                }
                const readResults = await Promise.all(readBatch.map((i) => this.execute(actions[i].toolName, actions[i].input, context)));
    ...
    executeSerial(tool, input, context) {
    ...
    async executeBlocking(tool, input, context) {
...
function buildToolCallAuditPayload(toolName, input, context) {
    const payload = {
        cwd: context.cwd,
        envDiff: context.env || {},
    };
...
function buildToolResultAuditPayload(toolName, result) {
    void toolName;
...
//# sourceMappingURL=tool-scheduler.js.map