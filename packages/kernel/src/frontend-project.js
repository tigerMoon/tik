...
function existingPaths(projectPath, candidates) {
    return candidates
...
function detectPackageManager(projectPath) {
    for (const [manager, files] of Object.entries(PACKAGE_LOCKFILES)) {
        if (manager === 'unknown')
            continue;
...
function detectFramework(args) {
    ...
    const hasConfig = (prefix) => configFiles.some((file) => file.startsWith(prefix));
    if (dependencies.has('next') || hasConfig('next.config'))
        return 'next';
    if (dependencies.has('nuxt') || hasConfig('nuxt.config'))
        return 'nuxt';
    if (dependencies.has('astro') || hasConfig('astro.config'))
        return 'astro';
    if (dependencies.has('vue') && dependencies.has('vite'))
        return 'vue-vite';
    ...
    if (dependencies.has('vue'))
        return 'vue';
    if (args.entrypoints.some((entry) => entry.endsWith('.tsx') || entry.endsWith('.jsx')))
        return 'frontend-tsx';
    return 'unknown';
}
function collectDesignSystemSignals(args) {
    ...
    if (dependencies.has('tailwindcss'))
        signals.push('tailwindcss');
    if (dependencies.has('@storybook/react') || args.storyRoots.length > 0)
        signals.push('storybook');
    if (dependencies.has('@radix-ui/react-slot') || dependencies.has('@headlessui/react'))
        signals.push('headless-ui');
    if (args.componentRoots.length > 0)
        signals.push('component-root');
    if (args.configFiles.some((file) => file.includes('tailwind.config')))
        signals.push('tailwind-config');
    return Array.from(new Set(signals));
}
export function inspectFrontendProject(projectPath) {
    ...
    if (entrypoints.length > 0)
        score += 2;
    if (componentRoots.length > 0 || styleRoots.length > 0)
        score += 1;
    return {
        framework,
        packageManager,
...
export function isLikelyFrontendTask(taskDescription, report) {
...
//# sourceMappingURL=frontend-project.js.map