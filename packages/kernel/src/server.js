...
import { canRetryWorkbenchTask, createEnvironmentPackSelection, generateId, isWorkbenchTaskCodexDispatchable, isWorkbenchTaskExternallyOwnedClaudeReview, isWorkbenchTaskMaintenance, isWorkbenchTerminalStatus, toEnvironmentPackSnapshot, } from '@tik/shared';
...
export async function createServer(kernel, config = { port: 3000, host: 'localhost' }, options) {
    ...
    function resolveWorkspaceRoot(rootPath) {
    ...
    function decorateWorkspaceApiReply(reply) {
        reply.header('X-Tik-Workspace-Api-Version', WORKSPACE_PUBLIC_API_VERSION);
    }
    async function resolveWorkspaceProject(rootPath, projectName, sourceProjectPath) {
    ...
    async function resolveWorkbenchWorkspaceBinding(rootPath, executionProjectPath, requested) {
        ...
        const matchedProject = requested?.projectName
            ? projects.find((project) => (project.projectName === requested.projectName
            ...
            : requested?.effectiveProjectPath
                ? projects.find((project) => ((project.effectiveProjectPath || project.projectPath) === requested.effectiveProjectPath
    ...
    async function resolveSafeWorkbenchWorkspaceBinding(rootPath, executionProjectPath, requested) {
    ...
    async function resolveMultiAgentWorkspaceBinding(requested) {
        if (!requested) {
            return undefined;
        }
    ...
    function buildWorkbenchTaskDescription(title, goal, adjustment) {
        return [
            `${title}: ${goal}`,
    ...
    async function resolveWorkbenchEnvironmentPackBinding(body) {
        ...
        if (boundPack) {
            ...
            if (invalidSkills.length > 0 || invalidKnowledge.length > 0) {
                throw new WorkbenchTaskError('invalid_environment_selection', `Invalid task configuration. Unknown skills: ${invalidSkills.join(', ') || 'none'}. Unknown knowledge: ${invalidKnowledge.join(', ') || 'none'}.`);
            }
        }
    ...
    async function launchWorkbenchFollowUpTask(workbench, sourceTask, adjustmentOverride) {
    ...
    async function steerAdjustedWorkbenchTask(workbench, task, adjustment) {
    ...
    // Control task
    fastify.post('/api/tasks/:id/control', async (req, reply) => {
        try {
    ...
    // Event stream (SSE)
    fastify.get('/api/tasks/:id/events', async (req, reply) => {
    ...
    fastify.get('/api/workbench/events', async (req, reply) => {
        reply.raw.writeHead(200, {
            'Content-Type': 'text/event-stream',
            'Cache-Control': 'no-cache',
            'Connection': 'keep-alive',
    ...
    fastify.post('/api/workbench/tasks', async (req, reply) => {
        ...
        const workspaceBinding = await resolveWorkbenchWorkspaceBinding(workspaceRoot, req.body.workspaceBinding?.effectiveProjectPath || kernel.projectPath, req.body.workspaceBinding);
        const kernelTask = kernel.taskManager.create({
    ...
    fastify.patch('/api/v1/tasks/:id', async (req, reply) => {
        ...
        try {
            ...
            if (body.workspaceBinding) {
                ...
                const workspaceRoot = options?.workspaceRoot || kernel.projectPath || process.cwd();
                body.workspaceBinding = await resolveSafeWorkbenchWorkspaceBinding(workspaceRoot, body.workspaceBinding.effectiveProjectPath || kernel.projectPath || process.cwd(), body.workspaceBinding);
            }
    ...
    fastify.post('/api/v1/tasks/:id/transitions', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/workbench/tasks/:id/control', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
        ...
        try {
            ...
            if (req.body?.type === 'resume' && existingTask.status === 'in_review') {
                throw new WorkbenchTaskError('human_review_resume_not_allowed', 'Cannot resume a task that is in human review. Open the review artifact and accept or request changes instead.');
            }
    ...
    fastify.post('/api/v1/tasks/:id/comments', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/tasks/:id/labels', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/tasks/:id/dependencies', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.get('/api/v1/tasks/:id/routing-preview', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/tasks/:id/run', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/multi-agent/workflows', async (req, reply) => {
        try {
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId', async (req, reply) => {
        try {
    ...
    fastify.patch('/api/v1/multi-agent/workflows/:workflowId', async (req, reply) => {
        try {
            ...
            if (req.body.status && req.body.status !== existing.status) {
                throw new MultiAgentCoordinationError('invalid_transition', 'Workflow status changes must be recorded through a codex-workflow decision.');
            }
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/decisions', async (req, reply) => {
        try {
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
            ...
            const recorded = await multiAgentStore.recordDecisionIfMatch(req.params.workflowId, req.body.decision, readIfMatch(req.headers['if-match']));
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/decisions/preflight', async (req, reply) => {
        try {
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/context-snapshots', async (req, reply) => {
        try {
            const result = await multiAgentStore.saveContextSnapshot(req.params.workflowId, req.body.snapshot, readIfMatch(req.headers['if-match']));
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/context-snapshots/:target', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/invocations/reconcile-stalled', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/human-overrides', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/actions/complete-subtask', async (req, reply) => {
        try {
            if (req.body.decision.action !== 'complete_subtask' || !req.body.decision.subtaskId) {
                throw new MultiAgentCoordinationError('invalid_transition', 'complete-subtask action requires a complete_subtask decision with subtaskId.');
            }
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
            ...
            const subtask = await multiAgentStore.updateSubtask(req.params.workflowId, req.body.decision.subtaskId, sanitizeSubtaskPatch({
                status: 'done',
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/actions/record-implementation', async (req, reply) => {
        try {
            if (req.body.decision.action !== 'record_implementation') {
                throw new MultiAgentCoordinationError('invalid_transition', 'record-implementation action requires a record_implementation decision.');
            }
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
            ...
            const subtask = req.body.decision.subtaskId
                ? await multiAgentStore.updateSubtask(req.params.workflowId, req.body.decision.subtaskId, sanitizeSubtaskPatch({
                    status: 'implemented',
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/actions/record-evaluation-result', async (req, reply) => {
        try {
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
            }
            let guard = { accepted: true, code: 'ok' };
            let decision;
            if (req.body.decision) {
                guard = evaluateWorkflowDecisionGuard(bundle, req.body.decision);
                if (!guard.accepted) {
                    reply.code(409);
                    return {
                        decision: req.body.decision,
            ...
            const evaluationRun = await multiAgentStore.recordEvaluationResult(req.params.workflowId, req.body.subtaskId, req.body.evaluationRunId, req.body.result);
            const subtask = req.body.subtaskPatch
                ? await multiAgentStore.updateSubtask(req.params.workflowId, req.body.subtaskId, sanitizeSubtaskPatch(req.body.subtaskPatch))
                : undefined;
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/actions/record-questioner-output', async (req, reply) => {
        try {
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
            ...
            const subtask = req.body.output.subtaskId && req.body.subtaskPatch
                ? await multiAgentStore.updateSubtask(req.params.workflowId, req.body.output.subtaskId, sanitizeSubtaskPatch(req.body.subtaskPatch))
                : undefined;
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/actions/complete-workflow', async (req, reply) => {
        try {
            if (req.body.decision.action !== 'complete_workflow') {
                throw new MultiAgentCoordinationError('invalid_transition', 'complete-workflow action requires a complete_workflow decision.');
            }
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/timeline', async (req, reply) => {
        try {
    ...
    fastify.put('/api/v1/multi-agent/workflows/:workflowId/task-graph', async (req, reply) => {
        try {
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/task-graph', async (req, reply) => {
        try {
            const bundle = await multiAgentStore.readBundle(req.params.workflowId);
            if (!bundle) {
                return sendV1Error(reply, 404, 'workflow_not_found', 'Multi-agent workflow not found');
    ...
    fastify.patch('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/contracts', async (req, reply) => {
        try {
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/contracts/latest', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/contracts/:contractId/accept', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/contracts/:contractId/stale', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/evaluations', async (req, reply) => {
        try {
    ...
    fastify.patch('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/evaluations/:evaluationRunId', async (req, reply) => {
        try {
            const evaluationRun = await multiAgentStore.updateEvaluationRun(req.params.workflowId, req.params.subtaskId, req.params.evaluationRunId, req.body);
            return { evaluationRun };
        }
        catch (error) {
            return sendV1CaughtError(reply, error);
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/evaluations/:evaluationRunId/result', async (req, reply) => {
        try {
            const evaluationRun = await multiAgentStore.recordEvaluationResult(req.params.workflowId, req.params.subtaskId, req.params.evaluationRunId, req.body.result);
            return { evaluationRun };
        }
        catch (error) {
            return sendV1CaughtError(reply, error);
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/evaluations/latest', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/subtasks/:subtaskId/evaluations/:evaluationRunId/validate-readonly', async (req, reply) => {
        try {
            const result = await multiAgentStore.validateEvaluationReadonly(req.params.workflowId, req.params.subtaskId, req.params.evaluationRunId, req.body);
            if (!result.guard.accepted) {
                reply.code(409);
            }
            return result;
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/questioner-outputs', async (req, reply) => {
        try {
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/questioner-outputs/latest', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/evidence', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/agent-invocations', async (req, reply) => {
        try {
    ...
    fastify.get('/api/v1/multi-agent/workflows/:workflowId/agent-invocations/:invocationId', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/agent-invocations/:invocationId/start', async (req, reply) => {
        try {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/agent-invocations/:invocationId/hook-start', async (req, reply) => {
        try {
            const invocation = await multiAgentStore.attestInvocationStart(req.params.workflowId, req.params.invocationId, req.body);
            return { invocation: sanitizeAgentInvocation(invocation) };
        }
        catch (error) {
            return sendV1CaughtError(reply, error);
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/agent-invocations/:invocationId/result', async (req, reply) => {
        try {
            if (!isMultiAgentInvocationTerminalStatus(req.body.status)) {
                throw new MultiAgentCoordinationError('invalid_invocation_status', 'Agent invocation result status must be completed, failed, or cancelled.');
            }
            const invocation = await multiAgentStore.updateInvocation(req.params.workflowId, req.params.invocationId, {
    ...
    fastify.post('/api/v1/multi-agent/workflows/:workflowId/agent-invocations/:invocationId/hook-stop', async (req, reply) => {
        try {
            const invocation = await multiAgentStore.attestInvocationStop(req.params.workflowId, req.params.invocationId, req.body);
            return { invocation: sanitizeAgentInvocation(invocation) };
        }
        catch (error) {
            return sendV1CaughtError(reply, error);
    ...
    fastify.post('/api/v1/agent-loop/review-rounds', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/worktree-review-rounds', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/fix-work-items', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/human-review-work-items', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/tasks/:id/stale', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/tasks/:id/claude-review-runs', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.post('/api/v1/agent-loop/tasks/:id/review-result', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            return sendV1Error(reply, 500, 'workbench_unavailable', 'Workbench service is unavailable');
        }
    ...
    fastify.get('/api/v1/tracker/state', async (_req, reply) => {
        try {
            ...
            const tasks = await kernel.workbench?.listTasks?.();
    ...
    fastify.put('/api/v1/workflow', async (req, reply) => {
        try {
    ...
    fastify.get('/api/workbench/tasks/:id/timeline', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/tasks/:id/decisions', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/decisions/:decisionId/resolve', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/retry', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/configuration', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
        ...
        const updatedTask = await workbench.updateTaskConfiguration(req.params.id, createEnvironmentPackSelection(pack, nextSelection), toEnvironmentPackSnapshot(pack));
        if (!updatedTask) {
            reply.code(404);
            return { error: 'Workbench task not found' };
        }
        kernel.taskManager.updateEnvironmentPackSelection?.(req.params.id, updatedTask.environmentPackSelection, updatedTask.environmentPackSnapshot);
        return { task: updatedTask };
    });
    fastify.post('/api/workbench/tasks/:id/brief', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/brief/revert', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/archive', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/artifacts', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/artifacts', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/tasks/:id/artifacts', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/tasks/:id/runs', async (req, reply) => {
        try {
    ...
    fastify.get('/api/workbench/tasks/:id/runs/:runId', async (req, reply) => {
        const runStore = new FileAgentRunStore(resolveServerWorkspaceRoot(options, kernel));
    ...
    fastify.get('/api/workbench/tasks/:id/runs/:runId/proof', async (req, reply) => {
    ...
    fastify.post('/api/workbench/tasks/:id/runs/:runId/proof/regenerate', async (_req, reply) => {
        reply.code(501);
        return { error: 'Run proof regeneration is not implemented yet.' };
    });
    fastify.post('/api/workbench/tasks/:id/artifacts/generate', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/artifacts/preview', async (req, reply) => {
        ...
        try {
            ...
            if (error.message.startsWith('Unsupported artifact extension')) {
                reply.code(415);
                return { error: error.message };
            }
            if (error.message.includes('must stay within the project root')) {
                reply.code(403);
                return { error: error.message };
            }
            reply.code(500);
    ...
    fastify.get('/api/workbench/artifacts/:artifactId', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/artifacts/:artifactId/versions', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/artifacts/:artifactId/versions', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/workbench/artifacts/:artifactId/versions/:versionId/preview', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
    ...
    fastify.post('/api/workbench/artifacts/:artifactId/accept', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/artifacts/:artifactId/reject', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/review/accept', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/tasks/:id/review/reject', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.post('/api/workbench/artifacts/:artifactId/archive', async (req, reply) => {
        const workbench = kernel.workbench;
        if (!workbench) {
            reply.code(500);
            return { error: 'Workbench service is unavailable' };
    ...
    fastify.get('/api/environment-packs', async (_req, reply) => {
        const registry = kernel.environmentPacks;
        if (!registry) {
            reply.code(500);
    ...
    fastify.get('/api/environment-packs/dashboard', async (_req, reply) => {
        ...
        return buildEnvironmentPackDashboard(options?.workspaceRoot || kernel.projectPath || process.cwd(), packs, activePack?.id || null, tasks);
    });
    ...
    fastify.post('/api/environment-packs/active', async (req, reply) => {
        const registry = kernel.environmentPacks;
        if (!registry) {
            reply.code(500);
            return { error: 'Environment pack registry is unavailable' };
    ...
    fastify.post('/api/skills/:id/draft', async (req, reply) => {
        try {
            return {
    ...
    fastify.post('/api/skills/:id/publish', async (req, reply) => {
        try {
            return {
    ...
    fastify.get('/api/workspace/status', async (req, reply) => {
        try {
    ...
    fastify.get('/api/workspace/board', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const readModel = new WorkspaceReadModel(resolveWorkspaceRoot(req.query.rootPath));
    ...
    fastify.get('/api/workspace/report', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const readModel = new WorkspaceReadModel(resolveWorkspaceRoot(req.query.rootPath));
    ...
    fastify.get('/api/workspace/memory', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const readModel = new WorkspaceReadModel(resolveWorkspaceRoot(req.query.rootPath));
    ...
    fastify.get('/api/workspace/decisions', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const readModel = new WorkspaceReadModel(resolveWorkspaceRoot(req.query.rootPath));
            const status = await readModel.readStatusView();
    ...
    fastify.get('/api/workspace/worktrees', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const readModel = new WorkspaceReadModel(resolveWorkspaceRoot(req.query.rootPath));
            const status = await readModel.readStatusView();
    ...
    fastify.post('/api/workspace/worktrees/create', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
    ...
    fastify.post('/api/workspace/worktrees/use', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const rootPath = resolveWorkspaceRoot(req.query.rootPath);
            ...
            const readModel = new WorkspaceReadModel(rootPath);
            const status = await readModel.readStatusView();
            const entry = status.worktrees.entries.find((item) => item.projectName === req.body.projectName
    ...
    fastify.post('/api/workspace/worktrees/remove', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const rootPath = resolveWorkspaceRoot(req.query.rootPath);
            const { orchestrator, settings, project } = await resolveWorkspaceProject(rootPath, req.body.projectName, req.body.sourceProjectPath);
    ...
    fastify.post('/api/workspace/decisions/:id/resolve', async (req, reply) => {
        try {
            decorateWorkspaceApiReply(reply);
            const rootPath = resolveWorkspaceRoot(req.query.rootPath);
            const orchestrator = new WorkspaceOrchestrator();
...
const COMMENT_INJECTION_STATES = new Set([
    'in_progress',
    'running',
    'waiting_for_user',
]);
function injectHumanCommentIntoActiveTask(kernel, task, body) {
...
function hasOpenWorkbenchAttempt(task) {
    return Boolean(task.attempts?.some((attempt) => attempt.kernelTaskId && !attempt.finishedAt));
}
async function assertWorkspaceBindingInsideRoot(workspaceRoot, binding) {
...
function buildTrackerStateSummary(tasks) {
    const summary = {
        activeCandidates: 0,
        activeRuns: 0,
        maintenance: 0,
...
function findProcessLine(lines, requiredParts) {
...
function getPreviewContentType(filePath) {
...
function assertPreviewExtensionAllowed(filePath) {
...
function renderMarkdownArtifactPreview(markdown, title) {
    return [
        '<!doctype html>',
        '<html>',
        '<head>',
...
function buildDefaultWorkspaceBinding(workspaceRoot, projectPath) {
    return {
        workspaceRoot,
    ...
    return new TrackerDaemon({
        ...
        launcher: new WorkbenchTrackerLauncher(input.workbench, {
            ...
            runTask: (task, runInput) => runWorkbenchKernelTaskInBackground(task, {
                ...
                runTask: (kernelTask) => input.kernel.runTask(kernelTask, 'single'),
            }),
            isRunActive: (taskId) => Boolean(input.kernel.getSession?.(taskId)),
            stopTask: (taskId, reason) => {
                try {
                    input.kernel.control(taskId, { type: 'stop', reason });
                }
                catch { }
            },
        }),
...
async function loadWorkspaceTrackerWorkflow(workspaceRoot) {
...
function workbenchTaskToTrackedTaskForWorkflow(task, defaultProjectPath, options = {}) {
...
function isWorkflowV2CandidateStatus(status) {
    return status === 'todo'
        || status === 'retry'
        || status === 'in_progress'
        || status === 'running'
...
function resolveServerWorkspaceRoot(options, kernel) {
...
function isPublicApiRoute(method, url) {
    if (method === 'OPTIONS')
        return true;
    const pathname = url.split('?')[0] || '/';
    return method === 'GET' && (pathname === '/api/health' || pathname === '/health');
}
function workflowV2SelectorSkipReason(workflow, task) {
...
function sendV1ErrorWithBody(reply, statusCode, code, message, body) {
    return {
        ...sendV1Error(reply, statusCode, code, message),
        ...body,
    };
}
function sendV1CaughtError(reply, error) {
...
function lastDecisionMatchesForPreflight(current, expected) {
...
function sanitizeSubtaskPatch(input) {
    const unknownKeys = Object.keys(input || {}).filter((key) => !ALLOWED_SUBTASK_PATCH_KEYS.has(key));
    if (unknownKeys.length > 0) {
        throw new MultiAgentCoordinationError('invalid_subtask_patch', `Unsupported subtask patch field${unknownKeys.length === 1 ? '' : 's'}: ${unknownKeys.join(', ')}`);
    }
    return input;
}
function isMultiAgentInvocationTerminalStatus(status) {
    return status === 'completed' || status === 'failed' || status === 'cancelled';
}
function extractTaskGraphFromInvocationResult(result) {
