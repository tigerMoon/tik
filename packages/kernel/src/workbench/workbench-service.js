import { canArchiveWorkbenchTask, canRetryWorkbenchTask, EventType, extractModifiedFilesFromEvidenceBody, generateId, isWorkbenchTerminalStatus, } from '@tik/shared';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import { renderArtifactTemplate, } from '../artifacts/artifact-templates.js';
...
export class WorkbenchService {
    ...
    constructor(options) {
        ...
        this.eventBus.onAny((event) => {
            this.eventQueue = this.eventQueue.then(() => this.handleEvent(event), () => this.handleEvent(event));
        });
    }
    async createTask(input, taskId = input.id || generateId()) {
    ...
    async appendTaskRun(taskId, run) {
    ...
    async transitionTask(taskId, to, input = {}) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
        ...
        if (from !== to && fromCanonical !== toCanonical && !allowed.some((status) => canonicalTransitionStatus(status) === toCanonical)) {
            throw new WorkbenchTaskError('transition_not_allowed', `Cannot transition task ${taskId} from ${from} to ${to}.`);
        }
        const updatedAt = new Date().toISOString();
    ...
    async appendAttempt(taskId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
    ...
    async finishAttempt(taskId, attemptNumber, outcome, error) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async addComment(taskId, input) {
        ...
        if (slashCommand) {
            const agentLoopCommandResult = await this.applyAgentLoopHumanReviewCommand(taskId, slashCommand.command, slashCommand.reason);
    ...
    async updateTaskTrackerMetadata(taskId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async setTaskDependencies(taskId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async createAgentLoopWorkItem(input) {
    ...
    async createReviewRound(input) {
        return this.createAgentLoopWorkItem({
            ...input,
            kind: 'claude_review',
        });
    }
    async createFixWorkItem(input) {
        return this.createAgentLoopWorkItem({
            ...input,
            kind: 'codex_fix',
        });
    }
    async createHumanReviewWorkItem(input) {
        return this.createAgentLoopWorkItem({
            ...input,
            kind: 'human_review',
        });
    }
    async markAgentLoopStale(taskId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async completeAgentLoopReview(taskId, reviewResult) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            throw new WorkbenchTaskError('task_not_found', `Workbench task not found: ${taskId}`);
        }
        ...
        if (metadata.headSha && normalizedReview.headShaReviewed !== metadata.headSha) {
            throw new WorkbenchTaskError('head_sha_mismatch', `Review head sha ${normalizedReview.headShaReviewed} does not match expected ${metadata.headSha}.`);
        }
        ...
        const nextStatus = nextPhase === 'needs_codex_fix' ? 'todo' : 'in_review';
        const reviewTask = {
            ...bundle.task,
            status: nextStatus,
    ...
    async advanceReviewLoopAfterRuntime(taskId, input) {
    ...
    async listArtifactVersions(id) {
        await this.drainEventQueue();
        return this.artifacts?.listVersions(id) || [];
    }
    async readArtifactPreview(artifactId, versionId) {
        await this.drainEventQueue();
    ...
    async generateArtifactForTask(taskId, template = 'task-review') {
        await this.drainEventQueue();
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            throw new WorkbenchTaskError('task_not_found', `Workbench task not found: ${taskId}`);
    ...
    async appendArtifactVersion(input) {
        if (!this.artifacts) {
            throw new WorkbenchTaskError('artifacts_unavailable', 'Artifact registry is unavailable.');
        }
    ...
    async rejectArtifact(id, reason, actor) {
        if (!this.artifacts) {
            throw new WorkbenchTaskError('artifacts_unavailable', 'Artifact registry is unavailable.');
        }
    ...
    async archiveArtifact(id, actor) {
        if (!this.artifacts) {
            throw new WorkbenchTaskError('artifacts_unavailable', 'Artifact registry is unavailable.');
        }
    ...
    async requestToolApproval(taskId, toolName) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async resolveDecision(taskId, decisionId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
    ...
    async updateTaskConfiguration(taskId, selection, environmentPackSnapshot) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async updateTaskBrief(taskId, input) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    async revertLastTaskAdjustment(taskId) {
        const bundle = await this.store.readTaskBundle(taskId);
        if (!bundle.task) {
            return null;
        }
    ...
    canArchiveAgentLoopHumanReview(task) {
    ...
    async projectTaskState(task, timeline = []) {
    ...
    shouldIgnoreEventForTask(status) {
        return isWorkbenchTerminalStatus(status);
    }
    async resolveWaitingDecision(task, updatedAt, nextStatus, event) {
    ...
    resolveDecisionStatus(nextStatus, event) {
    ...
    async findTaskByKernelTaskId(kernelTaskId) {
        const tasks = await this.store.listTasks();
        return tasks.find((task) => (task.runs?.some((run) => run.kernelTaskId === kernelTaskId)
            || task.attempts?.some((attempt) => attempt.kernelTaskId === kernelTaskId))) ?? null;
    }
    async mapTaskStatus(currentStatus, event, taskId) {
    ...
    projectAttemptFromEvent(task, event, createdAt) {
    ...
    async stopActiveAttemptIfNeeded(previousTask, nextTask, reason) {
    ...
    async cleanupInactiveTaskStateIfNeeded(task, actor) {
    ...
    resolveAgentLoopPayloadForRootTask(input, rootTask) {
    ...
    buildAgentLoopIdempotencyKey(input, maxRounds) {
    ...
    async findTaskByAgentLoopIdempotencyKey(idempotencyKey) {
        const tasks = await this.store.listTasks();
        return tasks.find((task) => task.agentLoop?.idempotencyKey === idempotencyKey) ?? null;
    }
    async findTaskByIdOrIdentifier(taskIdOrIdentifier) {
        const tasks = await this.store.listTasks();
        return tasks.find((task) => task.id === taskIdOrIdentifier
            || task.identifier === taskIdOrIdentifier
            || task.shortIdentifier === taskIdOrIdentifier) ?? null;
    }
    async applyAgentLoopHumanReviewCommand(taskId, command, reason) {
        const bundle = await this.store.readTaskBundle(taskId);
        ...
        const updatedAt = new Date().toISOString();
        const phase = command === 'approve'
            ? 'complete'
            : 'needs_codex_fix';
        const status = command === 'approve' ? 'completed' : 'todo';
        const updatedTask = {
            ...task,
            status,
    ...
    statusForAgentLoopRetryPhase(phase) {
        switch (phase) {
    ...
    async advanceAfterClaudeReviewRuntime(taskId, stdout, runId) {
        const bundle = await this.store.readTaskBundle(taskId);
        const task = bundle.task;
        if (!task) {
            return null;
        ...
        const nextPhase = decision === 'blocking'
            ? 'needs_codex_fix'
            : 'needs_human_review';
        const nextStatus = decision === 'blocking' ? 'todo' : 'in_review';
    ...
    async advanceAfterCodexFixRuntime(taskId) {
        const bundle = await this.store.readTaskBundle(taskId);
        const task = bundle.task;
        if (!task) {
            return null;
    ...
    labelsForAgentLoopPhase(phase) {
        return this.applyAgentLoopLabels(undefined, phase);
    }
    phaseForAgentLoopKind(kind) {
    ...
    summaryForAgentLoopPhase(phase, metadata) {
    ...
    timelineBodyForAgentLoopPhase(phase, metadata) {
        const headSha = metadata.headSha || metadata.changeRequest.headSha;
        switch (phase) {
            case 'needs_claude_review':
    ...
    applyAgentLoopLabels(labels, phase) {
    ...
    buildAgentLoopDescription(metadata) {
        return [
            `Kind: ${metadata.kind}`,
    ...
    formatBlockingIssues(blockingIssues) {
    ...
    buildReviewTimelineBody(reviewResult) {
        return [
    ...
    buildReviewResultComment(reviewResult, nextPhase) {
        return [
    ...
    async createFixTaskFromReview(metadata, parentTaskId, blockingIssues, workspaceBinding) {
    ...
    async createHumanReviewTaskFromReview(metadata, parentTaskId, blockingIssues = [], workspaceBinding) {
    ...
    async reparentTask(task, parentTaskId) {
        const bundle = await this.store.readTaskBundle(task.id);
        if (!bundle.task) {
            return task;
        }
    ...
    buildHighRiskDecision(taskId, toolName, createdAt) {
        return {
            id: generateId(),
            taskId,
    ...
    shouldSuppressTimelineSummary(type) {
        return (type === EventType.SESSION_MESSAGE
    ...
    buildRawTimelineItem(event, taskId, createdAt) {
    ...
    formatToolEvidenceBody(payload) {
        const sections = [`Tool: ${payload.toolName}`];
        if (payload.filesModified?.length) {
            sections.push([
                'Files modified:',
                ...payload.filesModified.map((filePath) => `- ${filePath}`),
            ].join('\n'));
        }
        if (payload.error) {
            sections.push(`Error:\n${payload.error}`);
    ...
    buildTaskEvidenceSummary(timeline, artifacts = []) {
        const rawItems = [...timeline]
            .filter((item) => item.kind === 'raw')
            .sort((left, right) => right.createdAt.localeCompare(left.createdAt));
        const modifiedFiles = new Set();
        const previewableArtifacts = new Set();
        let latestPreviewableArtifactPath;
        let latestPreviewableArtifactCreatedAt;
        let latestToolName;
    ...
    extractNamedSection(body, sectionName) {
    ...
    isPreviewableArtifactPath(filePath) {
    ...
    async registerPreviewableArtifacts(event, task, rawItem, createdAt) {
    ...
    async appendArtifactTimelineItem(artifact, action) {
        await this.store.appendTimelineItem({
            id: generateId(),
    ...
    async moveTaskToArtifactReview(artifact) {
    ...
    async completeTaskAfterArtifactAcceptance(artifact) {
        const bundle = await this.store.readTaskBundle(artifact.taskId);
        if (!bundle.task || bundle.task.status === 'cancelled' || bundle.task.status === 'archived') {
            return;
        }
    ...
    async reopenTaskAfterArtifactRejection(artifact, reason) {
        const bundle = await this.store.readTaskBundle(artifact.taskId);
        if (!bundle.task || bundle.task.status === 'cancelled' || bundle.task.status === 'archived') {
            return;
        }
    ...
    async hasPendingReviewArtifact(taskId) {
    ...
    buildArtifactProvenance(timeline) {
    ...
    isValidationToolEvidence(parsed) {
        if (!parsed.toolName) {
            return false;
        }
        const haystack = `${parsed.toolName}\n${parsed.output || ''}\n${parsed.error || ''}`.toLowerCase();
        return (parsed.toolName === 'bash'
    ...
    async assertNoDependencyCycle(taskId, nextBlockedByTaskIds) {
        const tasks = await this.store.listTasks();
        ...
        const visitsTarget = (currentId, seen) => {
            if (currentId === taskId) {
                return true;
            }
...
function canonicalTransitionStatus(status) {
    return TRANSITION_ALIASES[status] || status;
}
function classifyClaudeReviewOutput(stdout) {
    const normalized = stdout.toLowerCase();
    if (/\b(no|none|zero)\s+blocking\b/.test(normalized)
    ...
    if (normalized.includes('blocking finding')
...
function buildClaudeReviewComment(stdout, decision, runId) {
    return [
...
function applyReviewLoopLabels(labels, phase) {
    const managedLabels = new Set([
        'needs-claude-review',
        'claude-reviewing',
        'needs-codex-fix',
