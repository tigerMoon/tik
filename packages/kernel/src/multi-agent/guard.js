...
export function evaluateWorkflowDecisionGuard(bundle, decision) {
    ...
    if ((decision.action === 'request_re_review' || decision.action === 'request_claude_review')
        && round !== undefined
        && round > bundle.workflow.maxRounds) {
        return reject('max_rounds_exceeded', `Review round ${round} exceeds maxRounds=${bundle.workflow.maxRounds}.`, { round, maxRounds: bundle.workflow.maxRounds });
    }
    ...
    if (bundle.workflow.currentHeadSha && decisionHeadSha && decisionHeadSha !== bundle.workflow.currentHeadSha) {
        return reject('head_sha_mismatch', `Decision head sha ${decisionHeadSha} does not match workflow head sha ${bundle.workflow.currentHeadSha}.`, { expectedHeadSha: bundle.workflow.currentHeadSha, actualHeadSha: decisionHeadSha });
    }
...
function hasEvidence(evidence, evidenceId) {
    return evidence.some((item) => item.id === evidenceId);
}
function validateActionTransition(bundle, decision) {
...
function requireSubtaskStatus(bundle, decision, allowed) {
    ...
    if (!allowed.includes(subtask.status)) {
        return reject('invalid_transition', `Decision ${decision.action} cannot run when subtask ${decision.subtaskId} is ${subtask.status}.`, { subtaskId: decision.subtaskId, status: subtask.status, allowed });
    }
    return accept();
}
function guardCanExecuteSubtask(bundle, decision, allowed) {
    const statusGuard = requireSubtaskStatus(bundle, decision, allowed);
    if (!statusGuard.accepted)
        return statusGuard;
    if (!bundle.taskGraph || !decision.subtaskId)
        return accept();
    const contractGuard = requireAcceptedContractIfPolicyRequires(bundle, decision.subtaskId);
    if (!contractGuard.accepted)
...
function guardCompleteSubtaskV1(bundle, decision) {
    ...
    if (!evaluationEvidenceGuard.accepted)
        return evaluationEvidenceGuard;
    const decisionHeadSha = readStringInput(decision.inputs, 'currentHeadSha')
        || readStringInput(decision.inputs, 'headSha')
...
function requireIsolatedCodexSubagents(bundle, subtaskId, implementation, evaluation) {
    const builder = latestCompletedInvocation(bundle, subtaskId, (invocation) => invocation.role === 'executor'
    ...
    const evaluator = latestCompletedInvocation(bundle, subtaskId, (invocation) => invocation.role === 'evaluator'
...
function guardCompleteWorkflowV1(bundle, decision) {
    const evaluation = latestEvaluationRun(bundle, '__final__');
    if (!evaluation?.result) {
    ...
    if (!finalCoverageGuard.accepted)
        return finalCoverageGuard;
    return accept();
}
function requireFinalEvaluationCoversWorkflowContract(evaluation, bundle) {
    ...
    const missingCommands = contract.finalValidationCommands
        .filter((command) => !result.commandResults.some((commandResult) => commandResult.status === 'passed'
...
function buildFinalWorkflowContract(bundle) {
    return {
...
function normalizeGlobalAcceptanceCriteria(criteria) {
...
function requireAcceptedContractIfPolicyRequires(bundle, subtaskId) {
...
function latestPassedValidationForSubtask(bundle, subtaskId) {
    return latestEvidenceForSubtask(bundle, subtaskId, 'validation', (item) => item.passed === true);
}
function latestApprovedReviewForSubtask(bundle, subtaskId) {
...
function latestReviewEvidenceForSubtask(bundle, subtaskId) {
    return latestEvidenceForSubtask(bundle, subtaskId, 'review');
}
function plannedReviewEvidenceForDecision(bundle, decision) {
...
function plannedEvidenceRefsForDecision(bundle, decision) {
    const refs = new Set();
    if (['complete_subtask', 'fix_claude_blockers', 'validate_subtask', 'request_human_review'].includes(decision.action)) {
...
function latestEvidenceForSubtask(bundle, subtaskId, kind, predicate = () => true) {
    return bundle.evidence
        .filter((item) => item.subtaskId === subtaskId && item.kind === kind && predicate(item))
        .sort((left, right) => right.createdAt.localeCompare(left.createdAt))[0];
}
function latestCompletedInvocation(bundle, subtaskId, predicate) {
...
function guardCanRecordImplementation(bundle, decision) {
...
function guardCanRunCodexEvaluator(bundle, decision) {
    if (!decision.subtaskId) {
...
function guardCanQuestionEvaluation(bundle, decision) {
...
function guardCanFixEvaluationFindings(bundle, decision) {
...
function latestAcceptedContract(bundle, subtaskId) {
    return bundle.contracts
        .filter((contract) => contract.subtaskId === subtaskId && contract.status === 'accepted')
        .sort((left, right) => right.version - left.version || latestContractTime(right).localeCompare(latestContractTime(left)))[0];
}
function latestContractTime(contract) {
    return contract.acceptedAt || '';
}
function latestEvaluationRun(bundle, subtaskId) {
...
function hasBlockingQuestions(output) {
...
function requireEvaluationCoversContract(evaluation, contract) {
    const result = evaluation.result;
    if (!result) {
    ...
    const hasCommandEvidence = result.commandResults.some((command) => command.status === 'passed' || command.status === 'failed' || command.status === 'timeout');
...
function isRuntimeAttestedInvocation(invocation) {
...
function readChangedFiles(evidence) {
...
function readImplementationScopeCheck(evidence) {
    const scopeCheck = evidence.payload?.scopeCheck;
    if (!scopeCheck || typeof scopeCheck !== 'object')
        return undefined;
    return {
        allowed: scopeCheck.allowed === true,
        violations: Array.isArray(scopeCheck.violations)
            ? scopeCheck.violations.filter((entry) => typeof entry === 'string')
            : [],
    };
}
...
function usesCodexEvaluatorQuestionerGate(bundle) {
    const policy = bundle.workflow.policy;
    return Boolean(policy?.requireAcceptedContract
        || policy?.requireEvaluationPassForComplete
        || policy?.requireQuestionerAfterEvaluation);
}
function requireEvidenceHeadMatchesDecision(bundle, decision, evidence, label) {
    const decisionHeadSha = readStringInput(decision.inputs, 'currentHeadSha')
        || readStringInput(decision.inputs, 'headSha')
        || bundle.workflow.currentHeadSha;
...
function reviewResultPayload(evidence) {
    const result = evidence.payload?.result;
    return result && typeof result === 'object' && !Array.isArray(result)
        ? result
        : undefined;
}
function allSubtasksDone(bundle) {
...
function hasApprovedFinalReview(bundle) {
    return bundle.evidence.some((item) => {
        if (item.kind !== 'review' || item.subtaskId)
            return false;
        const result = reviewResultPayload(item);
        return result?.verdict === 'approve' && reviewBlockingIssueCount(item) === 0;
    });
}
function plannedFinalReviewApproved(decision) {
...
function hasPlannedReviewResult(decision) {
    return Boolean(readRecordInput(decision.inputs, 'plannedReviewResult'));
}
function hasTruthyInput(inputs, key) {
...
function validateWorkspaceBinding(workflow) {
...
function readStringInput(inputs, key) {
    const value = inputs?.[key];
    return typeof value === 'string' && value.trim() ? value.trim() : undefined;
}
function readNumberInput(inputs, key) {
    const value = inputs?.[key];
...
function readRecordInput(inputs, key) {
    const value = inputs?.[key];
    return value && typeof value === 'object' && !Array.isArray(value)
        ? value
        : undefined;
}
function reject(code, message, currentState) {
    return {
        accepted: false,
        code,
        message,
