...
export class WorkspaceSuperpowersClarifier {
    ...
    async clarify(input, now) {
        ...
        const method = category === 'approach' || input.phase === 'PARALLEL_PLAN'
            ? 'ralplan'
            : 'deep-interview';
...
function profileThreshold(profile) {
    if (profile === 'deep-verify')
        return 0.75;
    if (profile === 'fast-feedback')
        return 0.45;
    return 0.6;
}
function inferClarificationCategory(input, contractConfidence) {
    ...
    const threshold = profileThreshold(input.workflowProfile);
    if (/(don't assume|deep interview|clarify|澄清|不明确|不清楚)/i.test(text))
        return 'generic';
    if (/(方案|approach|option|tradeoff|取舍|A\/B|or |either |choose)/i.test(text))
        return 'approach';
    if (/(scope|范围|边界|ownership|归属|跨项目|谁来改|non-goal|out of scope)/i.test(text) && ambiguityCue)
        return 'scope';
    if (/(constraint|限制|依赖|dependency|兼容|回滚|rollback|前置)/i.test(text) && ambiguityCue)
        return 'constraints';
    if (/(test|测试|验证|验收|validation|coverage|用例)/i.test(text) && ambiguityCue)
        return 'validation';
    if (/(approve|approval|确认|是否继续)/i.test(text))
        return 'approval';
    if ((input.splitReason || '').toLowerCase().includes('no explicit project token matched'))
        return 'scope';
    if ((input.splitReason || '').toLowerCase().includes('multiple project tokens matched'))
        return 'scope';
    if (contractConfidence === undefined)
        return ambiguityCue ? 'generic' : 'skip';
    if (contractConfidence < threshold)
        return ambiguityCue ? 'generic' : 'skip';
    return 'skip';
}
function clarificationConfidence(contractConfidence, category) {
    if (category === 'approach')
        return 'medium';
    if (contractConfidence === undefined)
        return 'low';
    if (contractConfidence >= 0.75)
        return 'high';
    if (contractConfidence >= 0.5)
        return 'medium';
    return 'low';
}
...
function buildSkipRationale(contractConfidence, threshold, phase) {
    if (contractConfidence === undefined) {
...
function ensureClarifyDecisionDefaults(decision, projectName, category, recommendedNextPhase, confidence, rationale, signals) {
...
function categoryReason(category) {
    if (category === 'approach')
        return 'multiple viable approaches are visible';
    if (category === 'scope')
        return 'scope or ownership boundaries are still unclear';
    if (category === 'constraints')
        return 'key constraints are missing';
    if (category === 'validation')
        return 'validation targets are underspecified';
    if (category === 'approval')
        return 'an approval-style choice is still unresolved';
    return 'the demand is not yet concrete enough for a safe downstream handoff';
}
function detectDemandSignals(demand, summary) {
    const text = `${demand}\n${summary || ''}`.toLowerCase();
    return dedupe([
...
function buildClarifyDecisionOptions(projectName, category, recommendedNextPhase) {
...
function clarifyDecisionTitle(projectName, category) {
    if (category === 'scope')
    ...
    if (category === 'approach')
        return `Clarify approach for ${projectName}`;
    return `Clarify ${projectName}`;
}
function clarifyDecisionPrompt(projectName, category, recommendedNextPhase) {
    const nextPhase = phaseLabel(recommendedNextPhase);
    if (category === 'scope') {
...
//# sourceMappingURL=workspace-superpowers-clarifier.js.map