...
export function buildTaskContextSnapshot(
  task: WorkbenchTaskRecord | null | undefined,
...
export function humanCommentsBudget(
  comments: WorkbenchTaskCommentRecord[] | undefined,
): TaskOperatorComment[] | undefined {
  if (!comments || comments.length === 0) {
    return undefined;
  }

  const humanOnly = comments
    .filter((c): c is WorkbenchTaskCommentRecord & { authorKind: 'human' } => c.authorKind === 'human')
...
function buildLastAttemptSnapshot(task: WorkbenchTaskRecord): TaskContextSnapshot['lastAttempt'] {
...
function buildTimelineSummary(timeline: WorkbenchTimelineItem[]): string[] | undefined {
...
function truncateOptional(value: string | undefined, maxChars: number): string | undefined {
