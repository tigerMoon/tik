import type { AgentLoopMetadata, AgentRunRecord, RunEvent, CreateWorkbenchTaskInput, CreateTaskInput, TaskWorkspaceBinding, WorkbenchTaskBlockerRecord, WorkbenchTaskCommentRecord, WorkbenchTaskRecord, WorkbenchTaskRunRecord, WorkbenchTimelineItem } from '@tik/shared';
...
export interface TrackerWorkflowDefinition {
    ...
    renderPrompt(task: TrackedTask, input?: {
        attempt?: number;
...
export interface WorkbenchPort {
    ...
    transitionTask?(taskId: string, to: WorkbenchTaskRecord['status'], input?: {
        reason?: string;
        actor?: 'human' | 'agent' | 'daemon' | 'system';
