...
import { collectCompletionEvidence, summarizeCompletionEvidence, } from './workspace-completion-evidence.js';
import { WorkspaceSuperpowersClarifier } from './workspace-superpowers-clarifier.js';
function remapArtifactPathToExecutionRoot(artifactPath, sourceProjectPath, effectiveProjectPath) {
    if (!artifactPath)
        return undefined;
    if (artifactPath.startsWith(effectiveProjectPath))
        return artifactPath;
...
function pathRelativeSafe(from, to) {
    const relative = path.relative(from, to);
    if (!relative || relative.startsWith('..'))
        return undefined;
    return relative;
}
function joinPathSafe(root, relative) {
    return path.join(root, relative);
}
async function ensureExecutionTarget(services, input) {
...
function feedbackRetryCount(eventStore, phase, projectNames) {
    if (!eventStore?.count)
        return 0;
    return Math.max(0, ...projectNames.map((projectName) => eventStore.count?.({
        phase,
        projectName,
        kind: 'feedback.recorded',
    }) || 0));
}
async function maybeRecordFeedback(services, args) {
...
function projectStateByName(snapshot) {
    return new Map((snapshot.state?.projects || []).map((project) => [project.projectName, project]));
}
function recordArtifactCompletion(services, args) {
    services.eventStore?.record({
        level: 'project',
...
function slugifyClarificationProject(projectName) {
    return projectName.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'project';
}
async function buildClarificationArtifactPath(workspaceRoot, projectName) {
    ...
    async run(args) {
        ...
        for (const item of args.items) {
            ...
            if (clarification.needsClarification && clarification.decision) {
                await this.services.orchestrator.markClarifyBlocked(args.resolution.workspace.rootPath, item.projectName, artifactPath, clarification.summary, clarifyTaskId, 'awaiting_decision', clarification.decision);
                this.services.eventStore?.record({
                    level: 'project',
                    kind: 'phase.blocked',
                    phase: 'PARALLEL_CLARIFY',
                ...
                await this.services.orchestrator.markClarifyResult(args.resolution.workspace.rootPath, item.projectName, artifactPath, clarification.summary, clarifyTaskId, clarification.category === 'skip' ? 'skipped' : 'generated');
                this.services.eventStore?.record({
                    level: 'project',
                    kind: 'phase.completed',
                    phase: 'PARALLEL_CLARIFY',
    ...
    async run(args) {
        ...
        for (const item of args.items) {
            ...
            try {
                ...
                await this.services.orchestrator.markProjectBlocked(args.resolution.workspace.rootPath, item.projectName, 'PARALLEL_SPECIFY', `Unable to prepare execution path: ${message}`, projectState?.specTaskId);
                plans.push({ item });
                continue;
            }
            const expectedSpecPath = remapArtifactPathToExecutionRoot(projectState?.specPath, executionTarget.sourceProjectPath, executionTarget.effectiveProjectPath) || this.services.buildWorkspaceSpecTargetPath(executionTarget.effectiveProjectPath, item.projectName, item.demand);
            ...
            if (specResolution.ambiguous) {
                await this.services.orchestrator.markProjectBlocked(args.resolution.workspace.rootPath, item.projectName, 'PARALLEL_SPECIFY', `Multiple feature specs found; unable to choose automatically: ${specResolution.candidates.join(', ')}`, projectState?.specTaskId);
        ...
        const executableSpecs = plans.filter((plan) => Boolean(plan.spec)).map((plan) => plan.spec);
        ...
        if (prepared.handles.length > 0) {
            try {
                const executed = await Promise.race([
                    supervisor.executePrepared(prepared, async (record) => {
                        if (record.state === 'running') {
                            ...
                            await this.services.orchestrator.markProjectInProgress(args.resolution.workspace.rootPath, record.projectName, 'PARALLEL_SPECIFY', 'Delegating sdd-specify skill task.', record.taskId);
                            return;
                        }
                    ...
                    new Promise((_, reject) => {
                        setTimeout(() => reject(new Error(`Specify phase timed out after ${Math.floor(this.services.policyEngine.getPhaseBudgetMs('PARALLEL_SPECIFY') / 1000)}s.`)), this.services.policyEngine.getPhaseBudgetMs('PARALLEL_SPECIFY')).unref();
                    }),
                ]);
                for (const result of executed)
                    resultMap.set(result.projectName, result);
            }
            catch (error) {
                const reason = error instanceof Error ? error.message : String(error);
                await supervisor.cancelPrepared(prepared);
                for (const record of prepared.records)
                    monitor.onSubtaskFinished({ taskId: record.taskId });
                await this.services.killWorkspaceTaskProcesses(prepared.records.map((record) => record.taskId));
                ...
                for (const plan of plans) {
                    ...
                    if (!plan.spec)
                        continue;
                    const record = prepared.records.find((entry) => entry.projectName === plan.item.projectName);
                    try {
                        const rescueOutcome = await this.services.runNativeWorkspaceArtifactRescue(plan.spec, this.services.resolveNativeRescueProvider(phaseProvider), args.model, `Recovered target spec after delegated specify timeout: ${reason}`);
                        const rescuedSpecPath = (await this.services.resolveWorkspaceSpecArtifact(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, plan.spec.inputs.targetSpecPath || this.services.buildWorkspaceSpecTargetPath(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, plan.item.projectName, plan.item.demand))).path;
                        if (!rescuedSpecPath)
                            throw new Error('Native rescue completed without writing the target spec artifact.');
                        await this.services.orchestrator.markSpecifyResult(args.resolution.workspace.rootPath, plan.item.projectName, rescuedSpecPath, rescueOutcome.summary || 'Recovered workspace spec via native artifact rescue.', record?.taskId, 'native');
                        recordArtifactCompletion(this.services, {
                            phase: 'PARALLEL_SPECIFY',
                            projectName: plan.item.projectName,
                            taskId: record?.taskId,
        ...
        const planByName = new Map(plans.map((plan) => [plan.item.projectName, plan]));
        for (const plan of plans) {
            if (plan.existingSpecPath) {
            ...
            if (!phaseTask)
                continue;
            const nativeSpecPath = (await this.services.resolveWorkspaceSpecArtifact(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, this.services.buildWorkspaceSpecTargetPath(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, plan.item.projectName, plan.item.demand))).path;
            if (nativeSpecPath) {
            ...
            if (rescuePlan?.spec && this.services.policyEngine.shouldRunNativeArtifactRescue({ phase: 'PARALLEL_SPECIFY', artifactState: 'missing', timedOut: false, delegatedStatus: phaseTask.status })) {
                try {
                    const rescueOutcome = await this.services.runNativeWorkspaceArtifactRescue(rescuePlan.spec, this.services.resolveNativeRescueProvider(phaseProvider), args.model, `Recovered target spec after delegated specify completed without artifact (task status: ${phaseTask.status}).`);
                    const rescuedSpecPath = (await this.services.resolveWorkspaceSpecArtifact(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, rescuePlan.spec.inputs.targetSpecPath || this.services.buildWorkspaceSpecTargetPath(plan.executionTarget?.effectiveProjectPath || plan.item.projectPath, plan.item.projectName, plan.item.demand))).path;
                    if (rescuedSpecPath) {
    ...
    async run(args) {
        ...
        for (const item of args.items) {
            ...
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                await this.services.orchestrator.markProjectBlocked(args.resolution.workspace.rootPath, item.projectName, 'PARALLEL_PLAN', `Unable to prepare execution path: ${message}`, projectState?.planTaskId);
                plans.push({ item });
                continue;
            }
            const expectedSpecPath = remapArtifactPathToExecutionRoot(projectState?.specPath, executionTarget.sourceProjectPath, executionTarget.effectiveProjectPath) || this.services.buildWorkspaceSpecTargetPath(executionTarget.effectiveProjectPath, item.projectName, item.demand);
            const expectedPlanPath = remapArtifactPathToExecutionRoot(projectState?.planPath, executionTarget.sourceProjectPath, executionTarget.effectiveProjectPath) || this.services.buildWorkspacePlanTargetPath(executionTarget.effectiveProjectPath, item.projectName, item.demand);
            const specResolution = await this.services.resolveWorkspaceSpecArtifact(executionTarget.effectiveProjectPath, expectedSpecPath);
            if (specResolution.ambiguous) {
        ...
        if (prepared.handles.length > 0) {
            try {
                const executed = await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        setTimeout(() => reject(new Error(`Plan phase timed out after ${Math.floor(this.services.policyEngine.getPhaseBudgetMs('PARALLEL_PLAN') / 1000)}s.`)), this.services.policyEngine.getPhaseBudgetMs('PARALLEL_PLAN')).unref();
                    }),
                ]);
                for (const result of executed)
                    resultMap.set(result.projectName, result);
...
export class WorkspaceAcePhaseExecutor {
    ...
    async run(args) {
        ...
        const projectsMissingPlan = [];
        const executionPathBlockedProjects = [];
        const preflightResults = [];
        const preconvergedProjects = [];
        for (const item of args.items) {
            const projectState = stateByName.get(item.projectName);
            let executionTarget;
            try {
                ...
                await this.services.orchestrator.markProjectBlocked(args.resolution.workspace.rootPath, item.projectName, 'PARALLEL_ACE', `Unable to prepare execution path: ${message}`, projectState?.aceTaskId);
            ...
            if (evidence)
                preconvergedProjects.push({ item, evidence });
        }
        ...
        if (projectsMissingPlan.length > 0) {
            ...
            return {
                ...
                projectResults: [
                    ...preflightResults,
                    ...projectsMissingPlan.map((projectName) => ({
                        projectName,
                        status: 'blocked',
        ...
        try {
            executed = await Promise.race([
                ...
                new Promise((_, reject) => {
                    setTimeout(() => reject(new Error(`ACE phase timed out after ${Math.floor(this.services.policyEngine.getPhaseBudgetMs('PARALLEL_ACE') / 1000)}s.`)), this.services.policyEngine.getPhaseBudgetMs('PARALLEL_ACE')).unref();
                }),
            ]);
        }
        catch (error) {
            ...
            const phaseResults = [];
            for (const item of args.items) {
                const record = prepared.records.find((entry) => entry.projectName === item.projectName);
                const executionTarget = executionTargets.get(item.projectName);
                const aceSpec = this.services.contextAssembler.buildAceSubtaskSpec({
                    projectName: item.projectName,
        ...
        for (const item of args.items) {
            ...
            if (mappedStatus === 'failed' || mappedStatus === 'blocked')
                failedProjects.push(item.projectName);
            this.services.eventStore?.record({
                level: 'project',
...
//# sourceMappingURL=workspace-phase-executors.js.map