...
export function synthesizeWorkspaceDecision(input, now) {
    ...
    if (artifactOptions.length > 0) {
        ...
        return buildDecision(now, input, {
            ...
            prompt: buildPrompt(`Tik found multiple candidate ${artifactKind} for ${input.projectName}. Choose one artifact before rerunning ${phaseLabel(input.phase)}.`, semanticContext),
            options: artifactOptions,
            recommendedOptionId: artifactOptions.find((option) => option.recommended)?.id,
            allowFreeform: true,
            confidence: 'high',
            rationale: buildRationale('The blocker included multiple concrete artifact paths, so the safest recovery path is to pin one artifact explicitly before continuing.', semanticContext),
    ...
    if (approachOptions.length > 0) {
        return buildDecision(now, input, {
            ...
            prompt: buildPrompt(`Tik detected multiple viable approaches for ${input.projectName}. Pick one before continuing ${phaseLabel(input.phase)}.`, semanticContext),
            options: approachOptions,
            recommendedOptionId: approachOptions.find((option) => option.recommended)?.id,
            allowFreeform: true,
            confidence: 'medium',
            rationale: buildRationale('The blocker summary exposed multiple named approaches, so Tik converted them into explicit decision options instead of leaving the user with a raw text blocker.', semanticContext),
            signals: mergeSignals(semanticContext.signals, [
    ...
    if (input.blockerKind === 'REPLAN') {
        ...
        return buildDecision(now, input, {
            ...
            prompt: buildPrompt(`The current ${phaseLabel(input.phase)} result for ${input.projectName} needs a controlled reroute before the workspace can continue.`, semanticContext),
            options,
            recommendedOptionId: options[0]?.id,
            allowFreeform: true,
            confidence: 'medium',
            rationale: buildRationale('This blocker was classified as a replan condition, so the safest choices are to rerun the current phase or step back to an earlier planning phase.', semanticContext),
    ...
    if (input.blockerKind === 'EXECUTION_FAILED') {
        ...
        return buildDecision(now, input, {
            ...
            prompt: buildPrompt(`Execution failed during ${phaseLabel(input.phase)} for ${input.projectName}. Confirm whether Tik should retry this phase or step back.`, semanticContext),
            options,
            recommendedOptionId: options[0]?.id,
            allowFreeform: true,
            confidence: 'medium',
            rationale: buildRationale('Execution failures are recoverable, but Tik should not guess whether to retry immediately or step back without a human approval signal.', semanticContext),
    ...
    const clarificationSignals = mergeSignals(semanticContext.signals, detectClarificationSignals(summary, semanticContext.semanticText, clarificationCategory));
    ...
    return buildDecision(now, input, {
        ...
        prompt: buildPrompt(clarificationPrompt(input.phase, input.projectName, clarificationCategory), semanticContext),
        ...
        rationale: buildRationale(clarificationRationale(clarificationCategory, semanticContext), semanticContext),
        signals: clarificationSignals,
        sourceSummary: summary,
    });
}
function buildDecision(now, input, fields) {
    return {
        id: generateId(),
        status: 'pending',
        phase: input.phase,
...
function extractArtifactOptions(summary, phase) {
    if (!summary)
        return [];
...
function extractApproachOptions(summary, phase) {
    if (!summary)
        return [];
...
export function workspaceDecisionConfidenceRank(confidence) {
    if (confidence === 'high')
        return 3;
...
function buildPrompt(base, semanticContext) {
    if (semanticContext.contextSummary.length === 0)
        return base;
    return `${base} Context: ${semanticContext.contextSummary.join(' | ')}.`;
}
function buildRationale(base, semanticContext) {
    if (semanticContext.contextSummary.length === 0)
        return base;
    return `${base} Evidence: ${semanticContext.contextSummary.join(' | ')}.`;
}
function mergeSignals(base, extra) {
    return Array.from(new Set([...base, ...extra.filter(Boolean)]));
}
function inferClarificationCategory(input, semanticContext) {
...
function detectClarificationSignals(summary, semanticText, category) {
    const text = `${summary || ''}\n${semanticText}`;
    const signals = ['needs-clarification'];
...
function buildClarificationOptions(phase, category, projectName) {
...
function clarificationTitle(projectName, category) {
    if (category === 'scope')
        return `Clarify scope for ${projectName}`;
    if (category === 'constraints')
        return `Clarify constraints for ${projectName}`;
    if (category === 'validation')
        return `Clarify validation for ${projectName}`;
    if (category === 'approval')
        return `Confirm direction for ${projectName}`;
    return `Clarify ${projectName}`;
}
function clarificationPrompt(phase, projectName, category) {
    if (category === 'scope') {
...
function clarificationRationale(category, semanticContext) {
    if (category === 'scope') {
...
function clarificationConfidence(semanticContext) {
    if (semanticContext.evidenceCount >= 4)
        return 'high';
    if (semanticContext.evidenceCount >= 2)
        return 'medium';
    return 'low';
}
...
//# sourceMappingURL=workspace-decision-synthesizer.js.map