...
export class AgentCoordinator {
    ...
    agents = new Map();
    mode;
    constructor(eventBus, toolScheduler, contextBuilder, aceEngine, mode = 'single') {
    ...
    // ─── Single Agent Mode ────────────────────────────────────
    async executeSingleAgent(task, llm) {
        const loop = new AgentLoop(this.eventBus, this.toolScheduler, this.contextBuilder, llm, this.aceEngine);
        return loop.run(task);
    }
    // ─── Multi Agent Mode ─────────────────────────────────────
    async executeMultiAgent(task, defaultLLM) {
        ...
        const plannerLoop = new AgentLoop(this.eventBus, this.toolScheduler, this.contextBuilder, plannerLLM, this.aceEngine);
        ...
        const coderLoop = new AgentLoop(this.eventBus, this.toolScheduler, this.contextBuilder, coderLLM, this.aceEngine);
        ...
        if (this.agents.has('reviewer') && coderResult.status !== 'failed') {
            ...
            const reviewerLoop = new AgentLoop(this.eventBus, this.toolScheduler, this.contextBuilder, reviewerLLM, this.aceEngine);
            const reviewTask = {
                ...task,
