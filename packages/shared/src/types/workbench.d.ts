...
export type WorkbenchTaskStatus = 'new' | 'backlog' | 'todo' | 'in_progress' | 'in_review' | 'needs_review' | 'running' | 'waiting_for_user' | 'blocked' | 'verifying' | 'accepted' | 'rejected' | 'retry' | 'completed' | 'failed' | 'cancelled' | 'paused' | 'archived';
export type WorkbenchActor = 'user' | 'supervisor' | 'researcher' | 'coder' | 'reviewer' | 'system';
export type AgentLoopWorkItemKind = 'codex_implement' | 'codex_fix' | 'claude_review' | 'human_review';
...
export type ArtifactKind = 'run_review' | 'transcript' | 'html' | 'markdown' | 'svg' | 'json' | 'text' | 'diff' | 'validation_log' | 'agent_output' | 'user_deliverable' | 'diagnostic' | 'report' | 'dashboard' | 'checklist' | 'timeline' | 'comparison';
export type ArtifactStatus = 'draft' | 'previewable' | 'needs_review' | 'accepted' | 'rejected' | 'superseded' | 'archived';
export type ArtifactVisibility = 'local' | 'workspace' | 'project' | 'exported';
...
export declare function isWorkbenchTerminalStatus(status: WorkbenchTaskStatus): boolean;
export declare function canRetryWorkbenchTask(status: WorkbenchTaskStatus): boolean;
export declare function canArchiveWorkbenchTask(status: WorkbenchTaskStatus): boolean;
