...
import { canArchiveWorkbenchTask, canRetryWorkbenchTask, isWorkbenchTerminalStatus, } from '../src/index.js';
describe('workbench shared types', () => {
    ...
    it('models session, decision, and evidence records for an active task', () => {
        ...
        expect(decision.options.find((option) => option.id === 'approve')).toEqual(expect.objectContaining({ recommended: true }));
