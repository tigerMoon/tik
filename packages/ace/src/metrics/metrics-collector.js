...
export class MetricsCollector {
    iterationResults = new Map();
    constructor(eventBus) {
        if (eventBus) {
            eventBus.on(EventType.TOOL_RESULT, (event) => this.onToolResult(event));
            eventBus.on(EventType.TOOL_ERROR, (event) => this.onToolError(event));
            eventBus.on(EventType.ITERATION_STARTED, (event) => this.onIterationStart(event));
        }
    }
