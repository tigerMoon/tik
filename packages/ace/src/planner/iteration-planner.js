...
export class IterationPlanner {
    ...
    generateFixFirstPlan(input) {
        ...
        if (goals.length === 0)
            goals.push('General improvements');
        const plan = {
            goals,
    ...
    generateQualityFirstPlan(input) {
        const tasks = [];
        const goals = ['Improve overall code quality'];
        // Improve coverage
    ...
    generateBalancedPlan(input) {
        const tasks = [];
        const goals = ['Balanced improvement across all dimensions'];
        // Mix of fixes and improvements
        // Take top 2 failures
        for (const failure of input.failures.slice(0, 2)) {
            const type = failure.type === 'test' ? 'FIX_TEST' :
    ...
    scorePlan(plan, input) {
        ...
        const failureTaskCount = plan.tasks.filter(t => t.type.startsWith('FIX_')).length;
...
//# sourceMappingURL=iteration-planner.js.map