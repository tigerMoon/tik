export function buildWorkspaceHierarchy(tasks, workspaceStatus, fallbackRootPath) {
...
export function filterTasksByWorkspaceScope(tasks, scopeKey) {
...
export function buildWorkspaceBindingOptions(hierarchy) {
    return [
        {
            key: 'workspace',
...
export function resolveWorkspaceBindingOption(options, scopeKey) {
    return options.find((option) => option.key === scopeKey) || options[0];
}
export function buildProjectScopeKey(projectName, projectPath) {
    const pathToken = projectPath ? `:${projectPath}` : '';
    return `project:${projectName}${pathToken}`;
}
export function buildTaskBindingLabel(binding) {
    if (!binding) {
        return 'Workspace';
    }
    if (binding.projectName) {
...
//# sourceMappingURL=workspace-hierarchy.js.map