...
import { buildSkillManifestMutationInput, buildSkillBindingsSnippet, buildSkillChecklist, buildSkillChangeItems, buildSkillDependenciesSnippet, buildSkillImpactItems, buildSkillManifestRecords, buildSkillPublishMutationInput, buildSkillPublishRecommendation, buildSkillManifestSnippet, buildSkillTestHarnessSnippet, buildSkillVersionEntries, labelizeSkillId, resolveSkillManifestPersistenceStatus, } from './skills.js';
const BASE_PACK = {
...
describe('skills view models', () => {
    it('builds shared and environment-scoped skill manifests from packs and task selections', () => {
        const records = buildSkillManifestRecords([BASE_PACK, OPS_PACK], TASKS, 'commerce-ops');
    ...
    it('derives persistence state from saved draft and published registry entries', () => {
        const baseRecords = buildSkillManifestRecords([BASE_PACK, OPS_PACK], TASKS, 'commerce-ops');
    ...
    it('allows an initial publish when only a saved draft exists', () => {
        ...
        const draftSnapshot = buildSkillManifestMutationInput(currentApiCompat, 'Review portability before publish.').snapshot;
        const records = buildSkillManifestRecords([BASE_PACK, OPS_PACK], TASKS, 'commerce-ops', [
            {
                skillId: 'api-compat-check',
                ownerPackId: 'commerce-ops',
    ...
    it('uses none and patch bumps when published baseline only changes through notes', () => {
        ...
        const publishedSnapshot = buildSkillManifestMutationInput(currentPrReview, 'Stable baseline notes').snapshot;
        const records = buildSkillManifestRecords([BASE_PACK, OPS_PACK], TASKS, 'commerce-ops', [
            {
                skillId: 'pr-review',
                ownerPackId: 'commerce-ops',
