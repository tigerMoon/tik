export function classifyArtifactPreviewMode(artifact) {
...
export function shouldFetchArtifactPreviewText(mode) {
    return mode === 'diff' || mode === 'diff-stat' || mode === 'log' || mode === 'text';
}
export function parseArtifactDiff(content) {
    const files = [];
    let current = null;
...
export function parseArtifactDiffStat(content) {
    const rows = [];
    let summary;
...
export function parseArtifactLogSections(content) {
    const sections = [];
    let current = null;
    for (const line of splitLines(content)) {
...
function classifyDiffLine(line) {
    ...
    if (line.startsWith('-'))
        return 'remove';
    if (/^(index|new file|deleted file|similarity|rename from|rename to)\b/.test(line))
        return 'meta';
    return 'context';
}
...
//# sourceMappingURL=artifact-preview.js.map