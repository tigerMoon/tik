...
import { applyTaskAdjustmentPreset, allowedMetadataStatuses, buildRunProofPanelModel, buildTaskStatusBannerSpec, canArchiveWorkbenchTaskFromBanner, DASHBOARD_AGENT_LOOP_APPROVE_COMMENT, buildWorkbenchAcceptanceDigest, buildWorkbenchAcceptanceSummary, buildWorkbenchEvidenceDigest, buildWorkbenchLiveRunEntries, buildWorkbenchAgentLoopSummary, buildWorkbenchLatestCommentSummary, buildWorkbenchOperatorNoteSummary, buildWorkbenchQueueSignal, buildWorkbenchRuntimeControlActions, buildWorkbenchSteeringUpdateInput, buildWorkbenchTaskProgressColumns, buildWorkbenchTaskVisibleSummary, buildTimelineFeedMetrics, buildWorkbenchOverview, buildTaskAdjustmentPreview, buildWorkbenchFocusSummary, buildTimelineGroups, buildWorkbenchWorkspaceBindingSummary, filterStaleTimelineGroupsForTask, filterTimelineGroupsByLens, filterWorkbenchTasksByQuery, filterWorkbenchTasksByLens, filterVisibleWorkbenchTasks, getDefaultWorkbenchFeedLens, getLatestPreviewableArtifact, getNextActiveWorkbenchTaskId, getPreferredReviewArtifactId, groupWorkbenchTasks, normalizeWorkbenchSummaryText, parseWorkbenchEvidence, resolveWorkbenchLane, shouldLaunchWorkbenchFollowUp, sortWorkbenchTasks, } from './workbench.js';
...
describe('task status banner spec', () => {
    ...
    it('renders a decision-driven banner regardless of task status (no buttons)', () => {
        const spec = buildTaskStatusBannerSpec(baseTask({ status: 'running' }), [{ title: 'Approve high-risk bash', summary: 'Bash will run rm -rf node_modules' }]);
