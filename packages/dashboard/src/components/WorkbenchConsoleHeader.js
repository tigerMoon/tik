...
export function WorkbenchConsoleHeader({ packs, activePackId, activeTask, waitingCount, highRiskCount, selectedLens, bootstrapping = false, refreshing = false, liveStatus = 'idle', publishingReviewRound = false, onToggleFilter, onNewTask, onPublishReviewRound, onRefresh, }) {
    ...
    return (<header className="console-topbar">
      <div className="console-topbar-left">
        <div className="console-topbar-title-wrap">
          ...
          <div className="console-topbar-chips">
            {activePack ? (<span className="console-chip">{activePack.id}</span>) : null}
            {bootstrapping ? (<span className="console-chip">
      ...
      <div className="console-topbar-actions">
        <button type="button" className="console-ghost-button" onClick={onToggleFilter}>
          Filter
        </button>
        <button type="button" className="console-primary-button" onClick={onNewTask}>
          New task
        </button>
        <button type="button" className="console-ghost-button" disabled={publishingReviewRound} onClick={() => {
            void onPublishReviewRound?.();
        }}>
          {publishingReviewRound ? 'Publishing' : 'Publish review'}
        </button>
        <button type="button" className="console-ghost-button" aria-label="Sync workbench state" onClick={() => {
            void onRefresh?.();
        }} disabled={refreshing}>
...
//# sourceMappingURL=WorkbenchConsoleHeader.js.map