...
import { classifyArtifactPreviewMode, parseArtifactDiff, parseArtifactDiffStat, parseArtifactLogSections, shouldFetchArtifactPreviewText, } from '../view-models/artifact-preview';
export function ArtifactDetail({ artifact, versions, task, loading, busyAction, onAccept, onReject, onArchive, onOpenTask, }) {
    ...
    if (!artifact) {
        return (<section className="artifact-detail panel">
    ...
    return (<section className="artifact-detail panel">
      <header className="artifact-detail-header">
        ...
        <div className="artifact-action-row">
          <button type="button" className="console-secondary-button" disabled={busyAction !== null} onClick={() => void onAccept(artifact.id)}>
            {busyAction === 'accept' ? 'Accepting...' : 'Accept'}
          </button>
          <button type="button" className="console-secondary-button" disabled={busyAction !== null || !rejectionReason.trim()} onClick={() => void onReject(artifact.id, rejectionReason)}>
            {busyAction === 'reject' ? 'Rejecting...' : 'Request changes'}
          </button>
          <button type="button" className="console-danger-button" disabled={busyAction !== null} onClick={() => void onArchive(artifact.id)}>
      ...
      <div className="artifact-detail-grid">
        <main className="artifact-preview-pane">
          <div className="artifact-preview-toolbar">
            <label>
              <span>Version</span>
              <select value={activeVersion?.id || ''} onChange={(event) => setSelectedVersionId(event.target.value)}>
          ...
          {activeVersion ? (<ArtifactPreview artifact={artifact} mode={previewMode} previewUrl={previewUrl} previewText={previewText} previewError={previewError}/>) : (<div className="artifact-empty">No preview version available.</div>)}
        </main>
        ...
        <aside className="artifact-evidence-rail">
          ...
          <label className="artifact-reject-note">
            ...
            <textarea value={rejectionReason} onChange={(event) => setRejectionReason(event.target.value)} placeholder="Reason for request changes"/>
          </label>
...
function ArtifactPreview({ artifact, mode, previewUrl, previewText, previewError, }) {
    if (previewError) {
    ...
    switch (mode) {
        ...
        default:
            return (<iframe title={`${artifact.title} preview`} className="artifact-preview-frame" sandbox="allow-scripts" src={previewUrl}/>);
    }
}
function ArtifactDiffPreview({ content }) {
    const model = useMemo(() => parseArtifactDiff(content), [content]);
    return (<div className="artifact-formatted-preview artifact-diff-preview">
      {model.files.map((file, fileIndex) => (<section key={`${file.path}-${fileIndex}`} className="artifact-diff-file">
          <div className="artifact-diff-file-header">{file.path}</div>
          <pre>
            {file.lines.map((line, lineIndex) => (<span key={`${file.path}-${lineIndex}`} className={`artifact-diff-line is-${line.kind}`}>
                {line.text || ' '}
              </span>))}
          </pre>
        </section>))}
...
function ArtifactDiffStatPreview({ content }) {
    const model = useMemo(() => parseArtifactDiffStat(content), [content]);
    return (<div className="artifact-formatted-preview artifact-stat-preview">
      {model.summary ? <div className="artifact-stat-summary">{model.summary}</div> : null}
      {model.rows.length ? (<div className="artifact-stat-table" role="table" aria-label="Diff stat">
          {model.rows.map((row) => (<div key={row.filePath} className="artifact-stat-row" role="row">
        ...
        </div>) : (<ArtifactTextPreview content={content}/>)}
    </div>);
}
function ArtifactLogPreview({ content }) {
    const sections = useMemo(() => parseArtifactLogSections(content), [content]);
    return (<div className="artifact-formatted-preview artifact-log-preview">
      {sections.map((section) => (<section key={section.title} className="artifact-log-section">
          <div className="artifact-log-heading">{section.title}</div>
          <pre>
            {section.lines.map((line) => (<span key={line.number} className="artifact-log-line">
...
function ArtifactTextPreview({ content }) {
    return (<div className="artifact-formatted-preview artifact-text-preview">
      <pre>{content}</pre>
    </div>);
}
function ArtifactMetaList({ title, items }) {
    return (<div className="artifact-meta-list">
      <strong>{title}</strong>
      {items.length === 0 ? (<span>No entries</span>) : (<ul>
          {items.map((item) => <li key={item}>{item}</li>)}
        </ul>)}
    </div>);
}
//# sourceMappingURL=ArtifactDetail.js.map