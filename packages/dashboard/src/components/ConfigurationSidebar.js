...
export function ConfigurationSidebar({ task, packs }) {
    return (<aside style={{ width: '340px', background: 'var(--bg-sidebar)', borderLeft: '1px solid var(--glass-border)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div className="config-section">
        ...
        ].map(skill => (<div key={skill.name} className={`resource-chip ${skill.active ? 'active' : ''}`}>
              {skill.name}
      ...
      <div className="config-section" style={{ borderBottom: 'none' }}>
        ...
        ].map(kb => (<div key={kb.name} className="glass-panel" style={{ padding: '12px', background: 'rgba(2, 6, 23, 0.3)' }}>
...
//# sourceMappingURL=ConfigurationSidebar.js.map