...
function getEventColor(type) {
    if (type.includes('error') || type.includes('failed'))
        return '#ff6b6b';
    if (type.includes('completed') || type.includes('converge') || type.includes('result'))
        return '#51cf66';
    if (type.includes('started') || type.includes('called'))
        return '#74c0fc';
    if (type.includes('evaluation') || type.includes('fitness'))
        return '#ffd43b';
    return '#adb5bd';
}
...
export function ExecutionStream({ events }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div ref={containerRef} style={{
        ...
        {events.length === 0 && (<div style={{ color: '#666', padding: '20px', textAlign: 'center' }}>
            No events yet. Submit a task to start.
          </div>)}
        {events.map((event) => (<div key={event.id} style={{ display: 'flex', gap: '8px', padding: '2px 0' }}>
