...
export function TaskPanel({ tasks, activeTaskId, onSelectTask, onSubmitTask }) {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <form onSubmit={handleSubmit} style={{ padding: '8px 16px', borderBottom: '1px solid #333' }}>
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Describe a task..." style={{
            width: '100%',
            padding: '8px 12px',
            background: '#1a1a1a',
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#999' }}>
            <input type="radio" value="single" checked={mode === 'single'} onChange={(e) => setMode(e.target.value)}/>
            Single Agent
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#999' }}>
            <input type="radio" value="multi" checked={mode === 'multi'} onChange={(e) => setMode(e.target.value)}/>
            Multi Agent
          </label>
        </div>
      </form>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {tasks.length === 0 && (<div style={{ color: '#666', padding: '20px', textAlign: 'center', fontSize: '12px' }}>
            No tasks yet
          </div>)}
        {tasks.map((task) => (<div key={task.id} onClick={() => onSelectTask(task.id)} style={{
                padding: '10px 16px',
