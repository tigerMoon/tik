...
import { buildSkillBindingsSnippet, buildSkillChecklist, buildSkillChangeItems, buildSkillDependenciesSnippet, buildSkillImpactItems, buildSkillManifestRecords, buildSkillManifestSnippet, buildSkillPublishRecommendation, buildSkillTestHarnessSnippet, buildSkillVersionEntries, resolveSkillManifestPersistenceStatus, } from '../view-models/skills';
const SECTION_ITEMS = [
...
export function WorkbenchSkillsView({ packs, tasks, activePackId, activeTask, registryEntries, savingDraftSkillId = null, publishingSkillId = null, onSaveDraft, onPublish, onOpenTask, }) {
    const noteInputId = useId();
    const [selectedSkillId, setSelectedSkillId] = useState(null);
    const [manifestTab, setManifestTab] = useState('overview');
    const [selectedSection, setSelectedSection] = useState('overview');
    const [feedback, setFeedback] = useState(null);
    const [draftNotesBySkill, setDraftNotesBySkill] = useState({});
    const records = useMemo(() => buildSkillManifestRecords(packs, tasks, activePackId, registryEntries), [activePackId, packs, registryEntries, tasks]);
    useEffect(() => {
        if (records.length === 0) {
    ...
    if (!selectedSkill) {
        return (<div className="skills-main">
    ...
    const publishActionLabel = publishingSkillId === selectedSkill.id
        ...
        : canPublish
            ...
            : persistenceStatus === 'published'
                ? 'Published'
                : 'Publish';
    const handleSectionSelect = (section, tab) => {
    ...
    return (<div className="skills-main">
      <section className="panel topbar">
        <div className="top-left">
          ...
          <div className="chips">
            ...
            {publishRecommendation.canPublish && publishRecommendation.nextVersion !== selectedSkill.version ? (<span className="chip">
        ...
        <div className="actions">
          ...
          <button type="button" className="btn primary" disabled={!latestTask} onClick={() => {
            if (!latestTask) {
                return;
            }
      ...
      <section className="skills-content">
        <section className="card skills-menu-card">
          <div>
            ...
            <div className="pack-list skills-skill-list">
              {records.map((record) => (<button key={record.id} type="button" className={`pack skills-skill-item ${record.id === selectedSkill.id ? 'selected' : ''}`} onClick={() => {
          ...
          <div className="card skills-subcard">
            ...
            <div className="menu">
              {SECTION_ITEMS.map((section) => (<button key={section.id} type="button" className={`menu-item ${selectedSection === section.id ? 'active' : ''}`} onClick={() => handleSectionSelect(section.id, section.tab)}>
                  {section.label}
                </button>))}
            </div>
          </div>
          ...
          <div className="card skills-subcard">
            ...
            <div className="version-list">
              {versionEntries.map((entry) => (<div key={entry.id} className="vitem">
        ...
        <section className="card skills-manifest-card">
        ...
        ].map((tab) => (<button key={tab.id} type="button" className={`tab ${manifestTab === tab.id ? 'active' : ''}`} onClick={() => {
                setManifestTab(tab.id);
          ...
          {manifestTab === 'spec' ? (<div className="codebox">
              <div className="code-title">Contract</div>
              {buildContractSnippet(selectedSkill)}
            </div>) : null}

          {manifestTab === 'bindings' ? (<div className="codebox">
              <div className="code-title">Bindings</div>
              {bindingsSnippet}
            </div>) : null}

          {manifestTab === 'tests' ? (<div className="codebox">
          ...
          <div className="notes">
            ...
            <textarea id={noteInputId} className="skills-note-editor" value={currentNotes} onChange={(event) => {
        ...
        <section className="skills-right-card">
          <section className="card">
            ...
            <div className="action-list">
              <button type="button" className="item skills-inline-button" onClick={() => {
            setManifestTab('tests');
            setSelectedSection('tests');
          ...
          <section className="card">
            ...
            <div className="queue">
              {changeItems.map((item) => (<div key={item.id} className={`qitem skills-change-item is-${item.tone}`}>
          ...
          <section className="card">
            ...
            <div className="queue">
              {impactItems.map((item) => (<div key={item.title} className="qitem">
                  <strong>{item.title}</strong>
                  <div className="small">{item.detail}</div>
                </div>))}
            </div>
          ...
          <section className="card">
            ...
            {checklist.map((item) => (<div key={item.label} className="check">
            ...
            void handleManifestCopy();
              ...
              <button type="button" className="item skills-inline-button" disabled={!latestTask} onClick={() => {
            if (latestTask) {
                onOpenTask(latestTask.id);
            }
        }}>
          ...
          <section className="card">
            ...
            <div className="action-list">
              {selectedSkill.relatedTasks.length > 0 ? selectedSkill.relatedTasks.map((task) => (<button key={task.id} type="button" className="item skills-inline-button skills-related-task" onClick={() => onOpenTask(task.id)}>
                  <strong>{task.title}</strong>
                  <span>{task.id.slice(0, 8).toUpperCase()} · {task.status.replace(/_/g, ' ')}</span>
                </button>)) : (<div className="item">No bound tasks selected this skill yet.</div>)}
            </div>
          </section>
        </section>
      </section>
...
function buildContractSnippet(skill) {
    return [
...
function buildEvaluationSnippet(skill) {
    return [
        'success_criteria:',
...
function buildSuccessSnippet(skill) {
    return [
        'success_criteria:',
...
function buildDefaultSkillNotes(skill) {
...
function resolvePersistenceChip(skill, status) {
    if (status === 'published') {
        return {
...
function resolvePreferredSkillId(records, packs, activePackId, activeTask) {
...
function resolvePublishHeadline(strategy) {
    switch (strategy) {
        case 'initial':
...
function resolvePublishTone(strategy) {
    switch (strategy) {
        case 'none':
            return 'green';
        case 'major':
...
//# sourceMappingURL=WorkbenchSkillsView.js.map