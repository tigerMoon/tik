...
export function IterationHistory({ metrics }) {
    if (metrics.length === 0) {
        return (<div style={{ color: '#666', padding: '12px', fontSize: '12px', textAlign: 'center' }}>
        No iterations yet.
      </div>);
    }
    return (<div style={{ padding: '8px 12px', fontSize: '11px' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {metrics.map((m) => {
            ...
            return (<tr key={m.iteration} style={{ borderBottom: '1px solid #1a1a1a' }}>
...
//# sourceMappingURL=IterationHistory.js.map