...
import { buildEnvironmentActivationSummary, buildEnvironmentPromotionQueue, buildEnvironmentWorkflowCoverage, countEnvironmentPromotionItems, formatRelativeSyncTime, getEnvironmentPackStatusBadge, } from '../view-models/environment';
export function WorkbenchEnvironmentView({ packs, activePackId, tasks, lastSyncedAt, dashboard, onSwitchPack, onUsePackForNewTask, onOpenTask, }) {
    const [selectedPackId, setSelectedPackId] = useState(activePackId || packs[0]?.id || null);
    ...
    const dashboardSummary = useMemo(() => (selectedPack ? dashboard?.summaries.find((summary) => summary.packId === selectedPack.id) || null : null), [dashboard?.summaries, selectedPack]);
    const promotionQueue = dashboardSummary?.promotionQueue
        || (selectedPack ? buildEnvironmentPromotionQueue(selectedPack) : []);
    const workflowCoverage = useMemo(() => (selectedPack ? buildEnvironmentWorkflowCoverage(selectedPack) : []), [selectedPack]);
    ...
    const boundTasks = useMemo(() => dashboardSummary?.latestBoundTasks || tasks
        ...
        .slice(0, 4), [dashboardSummary?.latestBoundTasks, selectedPack?.id, tasks]);
    ...
    if (!selectedPack) {
        return (<div className="environment-main">
    ...
    return (<div className="environment-main">
      <section className="panel topbar">
        ...
        <div className="actions">
          ...
          <button type="button" className="btn primary" disabled={busyAction !== null || selectedPack.id === activePackId} onClick={handleSwitchPack}>
      ...
      <section className="environment-content">
        <section className="card environment-pack-column">
          ...
          <div className="pack-list">
            ...
            return (<button key={pack.id} type="button" className={`pack ${pack.id === selectedPack.id ? 'selected' : ''}`} onClick={() => setSelectedPackId(pack.id)}>
                  <div className="row">
        ...
        <div className="stack">
          <section className="card">
            ...
            <div className="environment-bound-task-list">
              {boundTasks.length > 0 ? boundTasks.map((task) => (<button key={task.id} type="button" className="environment-bound-task environment-bound-task-button" onClick={() => onOpenTask(task.id)}>
                ...
                </button>)) : (<div className="environment-bound-task is-empty">No current tasks are bound to this pack.</div>)}
            </div>
          ...
          <section className="card" id="environment-promotion-queue">
            ...
            <div className="queue">
              {promotionQueue.length > 0 ? promotionQueue.map((item) => (<div key={item.id} className="qitem">
                  <strong>{item.kind}</strong>
                  <div className="small">{item.detail}</div>
                </div>)) : (<div className="qitem">
          ...
          <section className="card">
            ...
            <div className="action-list">
              <button type="button" className="item environment-action-button" onClick={handleSwitchPack} disabled={busyAction !== null || selectedPack.id === activePackId}>
                {selectedPack.id === activePackId ? `Using ${selectedPack.id}` : `Switch to ${selectedPack.id}`}
              </button>
              <button type="button" className="item environment-action-button" onClick={() => {
            ...
            setFeedback('Scrolled to the promotion queue.');
              ...
              <button type="button" className="item environment-action-button" onClick={async () => {
      ...
      {manifestOpen ? (<section className="card environment-manifest-card">
          <div className="environment-manifest-header">
            <div>
              ...
              {dashboardSummary?.manifestPath ? (<div className="small">{dashboardSummary.manifestPath}</div>) : null}
            </div>
...
function EnvironmentItemCard({ title, items, }) {
    return (<div className="item-card">
      <div className="item-title">{title}</div>
      <div className="item-list">
        {items.length > 0 ? items.map((item) => (<div key={item} className="item">{item}</div>)) : (<div className="item">No entries</div>)}
      </div>
    </div>);
}
function EnvironmentWorkflowCard({ bindings, }) {
    return (<div className="item-card" id="environment-workflow-bindings">
      <div className="item-title">Workflow bindings · {bindings.length}</div>
      <div className="environment-workflow-list">
        {bindings.length > 0 ? bindings.map((binding) => (<div key={binding.workflow} className="environment-workflow-item">
          ...
          </div>)) : (<div className="item">No workflow bindings</div>)}
      </div>
    </div>);
}
function EnvironmentCoverageCard({ coverage, }) {
    ...
    return (<div className="item-card">
      ...
      <div className="environment-coverage-list">
        {coverage.length > 0 ? coverage.map((workflow) => (<div key={workflow.workflow} className="environment-coverage-item">
            ...
            <div className="environment-coverage-phase-list">
              {workflow.phases.map((phase) => (<div key={`${workflow.workflow}:${phase.phase}`} className="environment-coverage-phase">
          ...
          </div>)) : (<div className="item">No workflow bindings declared</div>)}
      </div>
    </div>);
}
function currentlyOpenMessage(isOpen, packId) {
    return isOpen
        ? 'Manifest panel hidden.'
        : `Manifest panel opened for ${packId}.`;
}
function getEnvironmentTaskTimestamp(task) {
    return 'lastProgressAt' in task ? task.lastProgressAt || task.updatedAt : task.updatedAt;
}
function humanizeEnvironmentTaskStatus(status) {
    switch (status) {
...
//# sourceMappingURL=WorkbenchEnvironmentView.js.map