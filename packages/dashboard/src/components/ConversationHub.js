...
export function ConversationHub({ task, timeline, decisions }) {
    return (<div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: '0 24px 24px 24px' }}>
      <div className="system-instructions" style={{ marginTop: '0' }}>
        ...
        <textarea placeholder="Set context for the current conversation..." defaultValue={task?.goal ? `Act as an expert assistant to achieve: ${task.goal}` : "Provide instructions to the agent..."} rows={3}/>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {timeline.map((item) => (<div key={item.id} className={`message-bubble message-${item.kind === 'summary' ? 'agent' : item.kind === 'decision' ? 'system' : 'user'}`}>
        ...
        {timeline.length === 0 && (<div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontStyle: 'italic' }}>
      ...
      <div style={{ marginTop: '16px' }}>
        <div style={{
          ...
          <textarea placeholder={`Message ${task?.currentOwner || 'Supervisor'}... (Ctrl+Enter to send)`} style={{
            width: '100%',
...
//# sourceMappingURL=ConversationHub.js.map