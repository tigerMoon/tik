...
export function ControlPanel({ taskId, onControl }) {
    const [constraint, setConstraint] = useState('');
    const [strategy, setStrategy] = useState('incremental');
    if (!taskId) {
        return (<div style={{ padding: '16px', color: '#666', fontSize: '12px' }}>
        Select a task to control.
      </div>);
    }
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px' }}>
      {/* Action buttons */}
      ...
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
        ...
        <ControlButton label="Apply" color="#74c0fc" onClick={() => onControl(taskId, { type: 'change_strategy', strategy })}/>
      </div>

      {/* Constraint injection */}
      <div style={{ display: 'flex', gap: '6px' }}>
        <input type="text" value={constraint} onChange={(e) => setConstraint(e.target.value)} placeholder="Inject constraint..." onKeyDown={(e) => {
        ...
        <ControlButton label="Inject" color="#da77f2" onClick={() => {
            if (constraint.trim()) {
                onControl(taskId, { type: 'inject_constraint', constraint: constraint.trim() });
                setConstraint('');
            }
