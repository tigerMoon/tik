...
export function TaskStatusBanner({ spec, busyActionId, onAction }: TaskStatusBannerProps) {
  if (!spec) {
    return null;
  }

  return (
    <section className={`task-detail-banner is-${spec.tone}`}>
      ...
      <div className="task-detail-banner-body">
        ...
        {spec.detail ? (
          <span className="task-detail-banner-detail">{spec.detail}</span>
        ) : null}
      </div>
      {spec.actions.length > 0 ? (
        <div className="task-detail-banner-actions">
