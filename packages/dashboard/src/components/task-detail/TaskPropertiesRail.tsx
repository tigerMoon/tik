...
export function TaskPropertiesRail({
  ...
  const guardSave = async <T,>(fn: () => Promise<T>): Promise<void> => {
    setSaving(true);
    try {
      await fn();
    } finally {
  ...
  const labelOptions = useMemo(
    () => buildWorkbenchLabelSelectOptions(labelEnvironment),
    [labelEnvironment],
  );

  return (
    <aside className={`task-detail-rail${wrapperClassName ? ` ${wrapperClassName}` : ''}`} aria-busy={saving}>
      <div className="task-detail-rail-section">
        ...
        <select
          ...
          value={task.status}
          disabled={saving}
          onChange={(event) => guardSave(() => onUpdateTaskMetadata(task, {
            status: event.target.value as WorkbenchTaskResponse['status'],
          }))}
        >
          {statuses.map((status) => (
      ...
      <div className="task-detail-rail-section">
        ...
        <ChipMultiSelect
          ...
          disabled={saving}
          onChange={(next) => guardSave(() => onUpdateTaskMetadata(task, { labels: next }))}
        />
      ...
      {agentLoopSummary ? (
        <div className="task-detail-rail-section">
          ...
          <div className="task-detail-rail-readonly">
            ...
            {task.agentLoop?.reviewResult ? (
              <span>verdict {task.agentLoop.reviewResult.verdict} · {task.agentLoop.reviewResult.blockingIssues.length} blocking</span>
            ) : null}
            {task.agentLoop?.stale ? (
              <span>stale expected {task.agentLoop.stale.expectedHeadSha} · actual {task.agentLoop.stale.actualHeadSha}</span>
            ) : null}
          </div>
        </div>
      ) : null}
      ...
      <div className="task-detail-rail-section">
        ...
        <div className="task-detail-rail-readonly">
          ...
          {lastAttempt ? (
            <span>attempt #{lastAttempt.attemptNumber} · {lastAttempt.outcome || 'running'}</span>
          ) : null}
        </div>
      </div>
      ...
      {(task.attempts || []).length > 0 ? (
        <div className="task-detail-rail-section">
          ...
          <ul className="task-detail-rail-list">
            {(task.attempts || []).slice().reverse().slice(0, 5).map((attempt) => (
              <li key={attempt.attemptNumber} className={`task-detail-rail-list-row outcome-${attempt.outcome || 'running'}`}>
      ...
      {runtimeControlActions.length > 0 ? (
        <div className="task-detail-rail-section task-detail-rail-controls">
          {runtimeControlActions.map((action) => (
            <button
              key={action.id}
              type="button"
              className={action.danger ? 'console-danger-button' : 'console-secondary-button'}
              disabled={controllingTaskAction !== null}
              onClick={() => onControlTask(task.id, action.id)}
            >
