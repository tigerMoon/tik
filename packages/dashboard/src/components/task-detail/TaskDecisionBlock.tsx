...
export function TaskDecisionBlock({
  task,
  decision,
  resolvingDecisionId,
  onResolveDecision,
}: TaskDecisionBlockProps) {
  ...
  return (
    <section className="task-detail-decision">
      ...
      {decision.summary ? (
        <p className="task-detail-decision-summary">{decision.summary}</p>
      ) : null}
        ...
        {decision.options.map((option) => {
          ...
          return (
            <button
              key={option.id}
              type="button"
              className={recommended ? 'task-launch-button' : 'console-secondary-button'}
              disabled={resolving || !onResolveDecision}
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setError((err as Error).message || 'Unable to resolve this decision.');
                }
              }}
            >
