...
export function TaskActivityBlock({ task, timeline }) {
    ...
    return (<section className="task-detail-activity">
      ...
      <div className="task-detail-activity-stream live-run-terminal" ref={liveRunScrollRef}>
        {liveRunEntries.length === 0 ? (<div className="live-run-empty">
            No runtime output yet. As the supervisor plans, calls tools, or pauses for review, the latest lines stream here.
          </div>) : (liveRunEntries.map((entry) => (<div key={entry.id} className={`live-run-row tone-${entry.tone}`}>
              ...
              {entry.detail ? (<div className="live-run-row-detail">{entry.detail}</div>) : null}
            </div>)))}
      </div>
    </section>);
}
...
//# sourceMappingURL=TaskActivityBlock.js.map