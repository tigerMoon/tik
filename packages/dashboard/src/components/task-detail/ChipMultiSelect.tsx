...
export function ChipMultiSelect({ values, options = [], placeholder, disabled, onChange }: ChipMultiSelectProps) {
  ...
  return (
    <div className="chip-multi-select-wrap">
      <div className={`chip-multi-select ${disabled ? 'is-disabled' : ''}`}>
        {values.map((value) => (
          <span key={value} className="chip-multi-select-chip">
            <span>{value}</span>
            <button
              type="button"
              className="chip-multi-select-remove"
              disabled={disabled}
        ...
        <input
          type="text"
          className="chip-multi-select-input"
          value={draft}
      ...
      {remainingOptions.length > 0 ? (
        <div className="chip-multi-select-options" aria-label="Suggested labels">
          {remainingOptions.map((option) => (
            <button
              key={option.value}
              type="button"
              className={`chip-multi-select-option tone-${option.tone || 'neutral'}`}
              disabled={disabled}
