...
export function TaskArtifactRail({ task, artifacts, loading, onGenerate, onOpenArtifact, }) {
    const model = useMemo(() => buildTaskArtifactRailModel(artifacts), [artifacts]);
    return (<section className="task-artifact-rail">
      ...
      {!model.latestArtifactId ? (<div className="task-artifact-empty">
          No review artifact yet. Generate one from current timeline and evidence.
        </div>) : (<div className="task-artifact-summary">
...
//# sourceMappingURL=TaskArtifactRail.js.map