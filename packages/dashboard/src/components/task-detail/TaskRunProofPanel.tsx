...
export function TaskRunProofPanel({
  task,
  artifacts,
  busyArtifactId,
  onAcceptArtifact,
  ...
  return (
    <section className="task-run-proof-panel">
      ...
      <div className="task-run-proof-links">
        {Object.entries(model.links).map(([key, link]) => link ? (
          <a
            key={key}
            href={buildWorkbenchArtifactVersionPreviewUrl(link.artifactId, link.versionId)}
            target="_blank"
            rel="noreferrer"
          >
            {labelForLink(key)}
      ...
      {model.changedFiles.length ? (
        <div className="task-detail-acceptance-list">
          {model.changedFiles.slice(0, 5).map((filePath) => (
      ...
      <div className="task-run-proof-actions">
        ...
        <button
          type="button"
          disabled={!model.canDecide || busy || !onAcceptArtifact}
          onClick={() => void onAcceptArtifact?.(model.reviewArtifactId)}
        >
      ...
      {model.canDecide ? (
        <label className="task-artifact-reject">
          ...
          <button
            ...
            disabled={busy || !reason.trim() || !onRejectArtifact}
            onClick={() => void onRejectArtifact?.(model.reviewArtifactId, reason)}
          >
            Send request
          </button>
        </label>
