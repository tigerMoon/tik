...
const COMPOSER_PLACEHOLDER =
  'Add a comment. Slash commands: /approve /done /retry /block /cancel';
...
export function TaskCommentsBlock({ task, timeline, onAddTaskComment }: TaskCommentsBlockProps) {
  ...
  const latestHumanComment = reverseChronological.find((comment) => (
    comment.authorKind === 'human' && comment.body.trim()
  ));
  ...
  return (
    <section className="task-detail-comments">
      ...
      {hasRunError ? (
        <div className="task-detail-comment-run-alert">
          <div>
          ...
          {latestHumanComment ? (
            <blockquote>{latestHumanComment.body}</blockquote>
          ) : null}
        </div>
      ) : null}

      <form
        className="task-detail-comments-form"
      ...
      {reverseChronological.length === 0 ? (
        <div className="task-detail-comments-empty">
          No comments yet. The first one starts the audit trail.
        </div>
      ) : (
        <ul className="task-detail-comments-list">
          {reverseChronological.map((comment) => (
            <li key={comment.id} className="task-detail-comments-row">
              ...
              {commentStates[comment.id]?.detail ? (
                <div className="task-detail-comment-state-detail">
                  {commentStates[comment.id]!.detail}
                </div>
              ) : null}
...
export function buildCommentProcessingStates(
  comments: WorkbenchTaskResponse['comments'] = [],
  timeline: WorkbenchTimelineResponseItem[],
): Record<string, CommentProcessingState> {
  return comments.reduce<Record<string, CommentProcessingState>>((states, comment) => {
    const subsequentItems = timeline.filter((item) => (
      item.createdAt > comment.createdAt && !isCommentTimelineItem(item)
    ));
    const hasQueuedFollowUp = subsequentItems.some((item) => (
      item.body.includes('Task state changed: completed -> todo')
      || item.body.includes('Human comment requested a follow-up run.')
    ));
    const hasProcessedEvidence = subsequentItems.some((item) => (
      item.kind === 'raw'
    ...
    const hasErrorEvidence = subsequentItems.some((item) => (
      item.kind === 'raw' && item.body.includes('\nError:\n')
    ));
...
function isMergeRequestComment(body: string): boolean {
  const normalized = body.toLowerCase();
  return (
    (normalized.includes('merge') || normalized.includes('合并'))
    && (
...
function hasMergeCompletionEvidence(body: string): boolean {
  const normalized = body.toLowerCase();
  return (
    normalized.includes('merge made by')
...
function isCommentTimelineItem(item: WorkbenchTimelineResponseItem): boolean {
