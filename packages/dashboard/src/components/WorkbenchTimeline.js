...
import { buildTimelineFeedMetrics, buildTimelineGroups, filterStaleTimelineGroupsForTask, filterTimelineGroupsByLens, getDefaultWorkbenchFeedLens, } from '../view-models/workbench';
import { DecisionCard } from './DecisionCard';
import { EvidencePanel } from './EvidencePanel';
export function WorkbenchTimeline({ task, items, decisions, resolvingDecisionId, onResolveDecision, }) {
    const [feedLens, setFeedLens] = useState('all');
    ...
    const groups = useMemo(() => {
        ...
        return filterStaleTimelineGroupsForTask(buildTimelineGroups(merged), task?.status).slice().reverse();
    ...
    return (<div className="console-conversation">
      ...
      <div className="console-conversation-toolbar">
        <div className="console-feed-filter-bar">
          {feedOptions.map((option) => (<button key={option.lens} type="button" className={`console-feed-filter ${feedLens === option.lens ? 'is-active' : ''}`} onClick={() => {
                setFeedPinned(true);
      ...
      <div className="console-message-scroll">
        {groups.length === 0 ? (<div className="console-conversation-empty">
            No adjustments or execution events yet. Use the task brief editor above to refine the mission and steer the next pass.
          </div>) : visibleGroups.length === 0 ? (<div className="console-conversation-empty">
            Nothing matches the current activity filter yet. Switch the feed lens to inspect a different slice of the mission history.
          </div>) : (<div className="console-message-stack">
            {visibleGroups.map((group) => {
                const decision = group.summary.decisionId ? decisionMap.get(group.summary.decisionId) : undefined;
                return (<section key={group.summary.id} className={`console-message-card actor-${group.summary.kind === 'decision' ? 'decision' : group.summary.actor}`}>
                  {group.summary.kind === 'decision' && decision ? (<DecisionCard decision={decision} resolving={resolvingDecisionId === decision.id} onResolve={task && onResolveDecision
                            ? async (body) => onResolveDecision(task.id, decision.id, body)
                            : undefined}/>) : (<div className="console-message-body">
...
//# sourceMappingURL=WorkbenchTimeline.js.map