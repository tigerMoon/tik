...
import { getWorkbenchLabelAction, getWorkbenchLabelActionDefinition, isWorkbenchTaskCodexDispatchable, isWorkbenchTaskMaintenance, isWorkbenchTerminalStatus, } from '@tik/shared';
import { addTrackerTaskComment, acceptWorkbenchArtifact, archiveWorkbenchArtifact, archiveWorkbenchTask, controlWorkbenchTask, createWorkbenchTask, createWorktreeReviewRound, fetchWorkbenchArtifact, fetchWorkbenchArtifacts, fetchWorkbenchArtifactVersions, fetchEnvironmentPackDashboard, fetchEnvironmentPacks, fetchSkillManifestRegistry, fetchTrackerState, fetchWorkbenchTaskArtifacts, fetchWorkbenchDecisions, fetchWorkbenchTasks, fetchWorkbenchTimeline, fetchWorkflowFile, fetchWorkspaceStatus, generateWorkbenchTaskArtifact, publishSkillManifest, rejectWorkbenchArtifact, retryWorkbenchTask, refreshTracker, resolveWorkbenchDecision, revertWorkbenchTaskBrief, saveSkillManifestDraft, saveWorkflowFile, subscribeToWorkbenchEvents, switchEnvironmentPack, transitionTrackerTask, updateTrackerTask, updateWorkbenchTaskBrief, updateWorkbenchTaskConfiguration, } from './api/client';
import { ArtifactDetail } from './components/ArtifactDetail';
import { ArtifactGallery } from './components/ArtifactGallery';
import { countWorkbenchArtifactGroupsByStatus, groupWorkbenchArtifactsForReview, } from './view-models/artifacts';
...
import { buildWorkbenchAgentLoopSummary, filterWorkbenchTasksByQuery, filterWorkbenchTasksByLens, getNextActiveWorkbenchTaskId, groupWorkbenchTasks, resolveWorkbenchLane, sortWorkbenchTasks, } from './view-models/workbench';
import { buildTaskBindingLabel, buildWorkspaceBindingOptions, buildWorkspaceHierarchy, filterTasksByWorkspaceScope, resolveWorkspaceBindingOption, } from './view-models/workspace-hierarchy';
...
export function App() {
    const { tasks, activeTaskId, timeline, decisions, packs, activePackId, setTasks, setActiveTask, setTimeline, setDecisions, setPacks, } = useStore();
    const [retryingTaskId, setRetryingTaskId] = useState(null);
    const [archivingTaskId, setArchivingTaskId] = useState(null);
    const [savingAdjustmentTaskId, setSavingAdjustmentTaskId] = useState(null);
    const [revertingAdjustmentTaskId, setRevertingAdjustmentTaskId] = useState(null);
    const [savingConfigurationTaskId, setSavingConfigurationTaskId] = useState(null);
    const [controllingTask, setControllingTask] = useState(null);
    ...
    const [selectedLens, setSelectedLens] = useState('inbox');
    const [activeSurface, setActiveSurface] = useState('workbench');
    const [workbenchView, setWorkbenchView] = useState('list');
    const [selectedScopeKey, setSelectedScopeKey] = useState('workspace');
    ...
    const [launcherSeed, setLauncherSeed] = useState({ packId: null, selection: null, source: 'active-pack' });
    const [searchQuery] = useState('');
    const [liveStatus, setLiveStatus] = useState('idle');
    const [bootstrappingWorkbench, setBootstrappingWorkbench] = useState(true);
    const [packsSyncedAt, setPacksSyncedAt] = useState(null);
    const [environmentDashboard, setEnvironmentDashboard] = useState(null);
    ...
    const [artifactBusyAction, setArtifactBusyAction] = useState(null);
    const [taskArtifactBusyId, setTaskArtifactBusyId] = useState(null);
    const [generatingTaskArtifact, setGeneratingTaskArtifact] = useState(false);
    const [savingSkillId, setSavingSkillId] = useState(null);
    const [publishingSkillId, setPublishingSkillId] = useState(null);
    ...
    const inactiveTaskIds = useMemo(() => tasks
        .filter((task) => task.status === 'cancelled' || task.status === 'archived')
        .map((task) => task.id), [tasks]);
    const artifactGroupCounts = useMemo(() => countWorkbenchArtifactGroupsByStatus(groupWorkbenchArtifactsForReview(artifacts, { inactiveTaskIds })), [artifacts, inactiveTaskIds]);
    ...
    const liveStatusLabel = liveStatus === 'live'
        ...
        : liveStatus === 'connecting'
            ...
            : liveStatus === 'offline'
                ? 'Offline'
                : 'Idle';
    const buildFocusedTaskLauncherSeed = () => ({
    ...
    const buildActivePackLauncherSeed = (packId = activePackId) => ({
        packId: packId || null,
        selection: null,
        source: 'active-pack',
    });
    const refreshWorkbench = async (options) => {
        if (!options?.silent) {
    ...
    const reloadTaskDetails = async (taskId) => {
    ...
    const refreshArtifacts = async (options) => {
        if (!options?.silent) {
    ...
    const reloadArtifactDetails = async (artifactId) => {
    ...
    const handleSaveWorkflowFile = async (content) => {
        const saved = await saveWorkflowFile(content);
        setWorkflowFile(saved);
    };
    const handleTaskMetadataChange = async (task, input) => {
    ...
    const handleAddTaskComment = async (task, body) => {
        const updatedTask = await addTrackerTaskComment(task.id, body);
        setTasks(tasks.map((currentTask) => (currentTask.id === updatedTask.id ? updatedTask : currentTask)));
        await Promise.all([
            reloadTaskDetails(task.id),
            refreshTrackerState(),
        ]);
    };
    const handleResolveDecision = async (taskId, decisionId, body) => {
    ...
    const handleApplyAdjustment = async (taskId, input) => {
    ...
    const handleCreateTask = async (title, goal, input) => {
    ...
    const handleArtifactAccept = async (artifactId) => {
    ...
    const handleArtifactReject = async (artifactId, reason) => {
    ...
    const handleArtifactArchive = async (artifactId) => {
    ...
    const handleGenerateTaskArtifact = async (taskId) => {
    ...
    return (<div className={`inbox-shell ${compact ? 'is-compact' : ''}`}>
      <aside className="inbox-sidebar panel">
        ...
        <button type="button" className="sidebar-compose-button" onClick={() => {
        ...
        <nav className="inbox-nav workspace-scope-nav">
          ...
          <button type="button" className={`inbox-nav-item workspace-scope-item ${activeSurface === 'workbench' && selectedScopeKey === 'workspace' ? 'is-active' : ''}`} onClick={() => {
            setActiveSurface('workbench');
        ...
        <nav className="inbox-nav project-scope-nav">
          ...
          {workspaceHierarchy.projects.length === 0 ? (<div className="sidebar-empty">No projects yet</div>) : workspaceHierarchy.projects.map((project) => (<button key={project.key} type="button" className={`inbox-nav-item project-scope-item ${activeSurface === 'workbench' && selectedScopeKey === project.key ? 'is-active' : ''}`} onClick={() => {
                setActiveSurface('workbench');
                setWorkbenchView('list');
        ...
        ].map((item) => (<button key={item.label} type="button" className={`inbox-nav-item ${item.active ? 'is-active' : ''}`} onClick={item.onClick}>
      ...
      <main className="inbox-main">
        {activeSurface === 'skills' ? (<WorkbenchSkillsView packs={packs} tasks={tasks} activePackId={activePackId} activeTask={activeTask} registryEntries={skillRegistry} savingDraftSkillId={savingSkillId} publishingSkillId={publishingSkillId} onSaveDraft={async (skillId, notes, skill) => {
            ...
            }}/>) : activeSurface === 'artifacts' ? (<section className="artifact-surface">
            <ArtifactGallery artifacts={artifacts} tasks={tasks} selectedArtifactId={activeArtifactId} loading={refreshingArtifacts} onSelectArtifact={(artifactId) => setActiveArtifactId(artifactId)} onOpenTask={(taskId) => {
                setActiveSurface('workbench');
                setWorkbenchView('detail');
                resetWorkbenchScroll();
                setAutoFocusLane(false);
            ...
            <ArtifactDetail artifact={activeArtifact} versions={artifactVersions} task={activeArtifact ? tasks.find((task) => task.id === activeArtifact.taskId) || null : null} loading={refreshingArtifacts} busyAction={artifactBusyAction} onAccept={handleArtifactAccept} onReject={handleArtifactReject} onArchive={handleArtifactArchive} onOpenTask={(taskId) => {
                setActiveSurface('workbench');
                setWorkbenchView('detail');
                resetWorkbenchScroll();
                setAutoFocusLane(false);
          ...
          </section>) : activeSurface === 'environments' ? (<WorkbenchEnvironmentView packs={packs} activePackId={activePackId} tasks={tasks} lastSyncedAt={packsSyncedAt} dashboard={environmentDashboard} onSwitchPack={async (packId) => {
                ...
                setLauncherOpen(false);
                setActiveTask(taskId);
            }}/>) : activeSurface === 'tracker' ? (<TrackerSurface trackerState={trackerState} workflowFile={workflowFile} tasks={tasks} onOpenTask={(taskId) => {
                setActiveSurface('workbench');
                setWorkbenchView('detail');
                resetWorkbenchScroll();
                setAutoFocusLane(false);
            ...
            <WorkbenchConsoleHeader packs={packs} activePackId={activePackId} activeTask={activeTask} waitingCount={waitingCount} highRiskCount={highRiskCount} selectedLens={selectedLens} bootstrapping={bootstrappingWorkbench} refreshing={refreshingWorkbench} liveStatus={liveStatus} onToggleFilter={() => {
                setAutoFocusLane(false);
            ...
            }} onPublishReviewRound={handlePublishReviewRound} publishingReviewRound={publishingReviewRound} onRefresh={() => refreshWorkbench()}/>

            <section className={`inbox-content ${workbenchView === 'list' ? 'is-list-view' : 'is-detail-view'}`}>
              {workbenchView === 'list' || !activeTask ? (<WorkbenchTaskList packs={packs} activePackId={activePackId} tasks={filteredTasks} activeTask={activeTask} activeTaskId={activeTaskId} selectedLens={selectedLens} loading={bootstrappingWorkbench && tasks.length === 0} launcherOpen={launcherOpen} launcherSeedPackId={launcherSeed.packId} launcherSeedSelection={launcherSeed.selection || null} launcherSeedSource={launcherSeed.source} bindingOptions={bindingOptions} selectedBindingKey={selectedScopeKey} onSelectTask={(taskId) => {
                    setActiveTask(taskId);
                    if (taskId) {
                        setWorkbenchView('detail');
                        resetWorkbenchScroll();
                    ...
                    setLauncherOpen(nextOpen);
                }}/>) : (<TaskDetailPanel task={activeTask} pack={activeTaskPack} packs={packs} timeline={timeline} decisions={decisions} artifacts={taskArtifacts} resolvingDecisionId={resolvingDecisionId} retrying={activeTask ? retryingTaskId === activeTask.id : false} archiving={activeTask ? archivingTaskId === activeTask.id : false} savingAdjustment={activeTask ? savingAdjustmentTaskId === activeTask.id : false} revertingAdjustment={activeTask ? revertingAdjustmentTaskId === activeTask.id : false} savingConfiguration={activeTask ? savingConfigurationTaskId === activeTask.id : false} controllingTaskAction={activeTask && controllingTask?.taskId === activeTask.id ? controllingTask.action : null} timelineError={timelineError} onRetryTask={async (task) => {
                ...
                }} onApplyTaskAdjustment={(task, input) => handleApplyAdjustment(task.id, input)} onRevertLastAdjustment={async (task) => {
                    ...
                    try {
                        ...
                        const nextTasks = await fetchWorkbenchTasks();
                        const lensTasks = filterWorkbenchTasksByLens(filterWorkbenchTasksByQuery(nextTasks, searchQuery), selectedLens);
                ...
                }} onResolveDecision={handleResolveDecision} onSaveTaskConfiguration={async (taskId, selection) => {
                ...
                }} onUpdateTaskMetadata={handleTaskMetadataChange} onAddTaskComment={handleAddTaskComment} onGenerateArtifact={handleGenerateTaskArtifact} onAcceptArtifact={handleArtifactAccept} onRejectArtifact={handleArtifactReject} onOpenArtifact={(artifactId) => {
                    setActiveArtifactId(artifactId);
                    setActiveSurface('artifacts');
                    setLauncherOpen(false);
                }} generatingArtifact={generatingTaskArtifact} busyArtifactId={taskArtifactBusyId} onControlTask={async (taskId, action) => {
...
function TrackerSurface({ trackerState, workflowFile, tasks, onOpenTask, onRefresh, onSaveWorkflow, }) {
    ...
    const allActiveRunTasks = sortedTasks.filter((task) => (isTrackerActiveRunTask(task)
    ...
    const allFailedTasks = sortedTasks.filter((task) => (task.status === 'failed'
    ...
    const renderTaskRow = (task, options = {}) => {
        ...
        return (<button key={task.id} type="button" className={`tracker-task-row tone-${options.tone || trackerStatusTone(task.status)}`} onClick={() => onOpenTask(task.id)}>
    ...
    return (<section className="tracker-surface">
      <div className="tracker-surface-header">
        <div>
          <div className="queue-card-kicker">Tracker</div>
          <h1>Tik task daemon</h1>
        </div>
        <button type="button" className="task-launch-button" disabled={refreshing} onClick={async () => {
            setRefreshing(true);
            try {
                await onRefresh();
            }
      ...
      <section className="focus-lower-card tracker-panel tracker-listeners-panel">
        ...
        <div className="tracker-listener-list">
          {listeners.length === 0 ? (<div className="tracker-empty-row">Listener state has not been reported yet.</div>) : listeners.map((listener) => (<div key={listener.id} className={`tracker-listener-row tone-${trackerListenerTone(listener.status)}`}>
      ...
      <div className="tracker-dashboard-grid">
        <section className="focus-lower-card tracker-panel tracker-panel-primary">
          ...
          <div className="tracker-task-list">
            {dispatchCandidates.length === 0 ? (<div className="tracker-empty-row">No Codex-ready task is waiting.</div>) : dispatchCandidates.map((task) => renderTaskRow(task, {
        ...
        <section className="focus-lower-card tracker-panel">
          ...
          <div className="tracker-task-list">
            {activeRunTasks.length === 0 && staleTasks.length === 0 ? (<div className="tracker-empty-row">No task is currently running.</div>) : (<>
        ...
        <section className="focus-lower-card tracker-panel">
          ...
          <div className="tracker-task-list">
            {maintenanceTasks.length === 0 ? (<div className="tracker-empty-row">No workspace maintenance task is waiting.</div>) : maintenanceTasks.map((task) => renderTaskRow(task, {
            tone: 'neutral',
        ...
        <section className="focus-lower-card tracker-panel">
          ...
          <div className="tracker-attention-list">
            {attentionCount === 0 ? (<div className="tracker-empty-row">No retry, stale, or review handoff is pending.</div>) : (<>
                {retryEntries.slice(0, 4).map((retry) => {
                const task = tasks.find((candidate) => candidate.id === retry.taskId);
                return (<button key={`retry-${retry.taskId}`} type="button" className="tracker-attention-row tone-yellow" onClick={() => task ? onOpenTask(task.id) : undefined}>
                      <span>
      ...
      <section className="focus-lower-card tracker-panel tracker-events-panel">
        ...
        <div className="tracker-event-feed">
          {recentActivity.length === 0 ? (<div className="tracker-empty-row">No tracker event has been recorded yet.</div>) : recentActivity.map((entry) => (<div key={`${entry.createdAt}-${entry.type}-${entry.shortIdentifier}`} className="tracker-event-row">
      ...
      <details className="focus-lower-card tracker-workflow-editor">
        ...
        <div className="tracker-workflow-body">
          <div className="tracker-workflow-header">
            ...
            <button type="button" className="task-launch-button" disabled={savingWorkflow || !workflowDraft.trim()} onClick={async () => {
          ...
          <textarea className="tracker-workflow-textarea" value={workflowDraft} onChange={(event) => setWorkflowDraft(event.target.value)} spellCheck={false}/>
        </div>
      </details>
    </section>);
}
function isTrackerDispatchCandidate(task) {
    if (task.status !== 'todo' && task.status !== 'failed') {
        return false;
    }
    return isWorkbenchTaskCodexDispatchable(task);
}
function isTrackerMaintenanceTask(task) {
...
function isWaitingForClaudeReview(task) {
...
function isWaitingForHumanReview(task) {
    if (isWorkbenchTerminalStatus(task.status)) {
        return false;
    }
...
function humanizeTrackerStatus(status) {
    switch (status) {
...
function trackerListenerTone(status) {
    switch (status) {
        case 'running':
            return 'green';
        case 'expected':
...
function humanizeTrackerListenerStatus(status) {
    switch (status) {
        case 'running':
            return 'Running';
        case 'expected':
...
function formatTrackerRelativeTime(value) {
    if (!value) {
        return 'No activity';
    }
...
function formatTrackerDueTime(dueAtMs) {
