...
function shouldFallbackToNonStreaming(error) {
    ...
    return (lowered.includes('unexpected event order')
...
export class ClaudeLLMProvider {
    ...
    constructor(model = 'claude-sonnet-4-6') {
        ...
        this.streamTimeoutMs = parseTimeoutMs(process.env.TIK_CLAUDE_STREAM_TIMEOUT_MS, DEFAULT_STREAM_TIMEOUT_MS);
        this.streamIdleTimeoutMs = parseTimeoutMs(process.env.TIK_CLAUDE_STREAM_IDLE_TIMEOUT_MS, DEFAULT_STREAM_IDLE_TIMEOUT_MS);
    }
...
When you have enough understanding, respond with a text plan summary (no more tool calls).`;
        ...
        for (let turn = 0; turn < maxTurns; turn++) {
            ...
            const toolResults = [];
            for (const block of toolUseBlocks) {
                const result = await this.executePlanTool(block.name, block.input, allowedTools, options?.cwd || process.cwd());
                allActions.push({
                    tool: block.name,
                    input: block.input,
     ...
     */
    async executePlanTool(name, input, allowedTools = new Set(PLAN_READ_ONLY_TOOL_POLICY.allowedTools), cwd = process.cwd()) {
            ...
            try {
                ...
                if (streamTimedOut) {
                    try {
                        ...
                        throw new Error(`Claude stream timed out: ${timeoutReason}. Last activity was ${Date.now() - lastActivityAt}ms ago.`);
                    }
                }
...
async function resolvePlanToolPath(cwdInput, requestedPath) {
...
async function resolveExistingPlanToolParent(cwd, targetDir) {
...
async function resolvePlanToolTarget(resolved) {
    try {
