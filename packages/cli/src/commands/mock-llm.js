...
export class MockLLMProvider {
    name = 'mock';
    async plan(prompt, _context) {
        const taskText = extractLineValue(prompt, 'Task:') || prompt;
        const syntheticMessages = [{ role: 'user', content: taskText }];
    ...
    async chatWithContext(messages, _systemPrompt, _context, _tools) {
...
function buildMockSpecDocument(prompt) {
...
function buildMockPlanDocument(prompt) {
    const demand = extractLineValue(prompt, 'Demand:') || 'Implement the requested workspace feature.';
...
function extractLineValue(prompt, prefix) {
    return prompt
        .split('\n')
        .map((line) => line.trim())
...
function isImplementationRequest(messages) {
...
function canWrite(availableTools) {
    return availableTools.has('write_file') || availableTools.has('edit_file');
}
function isHighRiskApprovalRequest(messages) {
    const lowered = messages
        .filter((message) => message.role === 'user')
        .map((message) => message.content)
        .join('\n')
        .toLowerCase();
    return (lowered.includes('发布')
        || lowered.includes('上线')
        || lowered.includes('deploy')
        || lowered.includes('publish')
        || lowered.includes('release'));
}
function buildMockWriteArguments(messages) {
...
function inferMockWritePath(taskText) {
...
function buildMockImplementationContent(taskText) {
    const normalizedTask = taskText.replace(/^task:\s*/i, '').trim();
    if (normalizedTask.includes('贪吃蛇')
    ...
    if (normalizedTask.toLowerCase().includes('h5')
...
function buildMockSnakeGameHtml(taskTitle) {
    return [
        '<!doctype html>',
        '<html lang="zh-CN">',
        '<head>',
