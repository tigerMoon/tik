...
export function buildDelegateImplementationContract(taskDescription, context, runtimeSystemMessages, writeToolsAvailable) {
    ...
    if (keyFiles.length > 0) {
        ...
        for (const line of keyFiles)
            sections.push(`- ${line}`);
    }
    if (pendingWork.length > 0) {
        sections.push('', '## Pending Work');
        for (const line of pendingWork)
            sections.push(`- ${line}`);
    }
    if (currentFocus.length > 0) {
        sections.push('', '## Current Focus');
        for (const line of currentFocus)
            sections.push(`- ${line}`);
    }
...
export function buildDelegateDocumentationContract(taskDescription, context, writeToolsAvailable) {
    const keyFiles = extractDelegateHints(context, ['Key files:', '- Key files:', 'Likely Target Paths:', '- Likely Target Paths:']);
...
function extractArtifactTargets(taskDescription, context) {
...
export function shouldStopForPostPatchValidation(normalizedCommand, patchFirstRequired, hasWorkspaceChanges, validationAttemptsAfterPatch, exitCode) {
...
export function shouldStopForPostPatchReadLoop(toolName, hasWorkspaceChanges, currentChangedFileCount, lastChangedFileCount, postPatchReadOnlySteps) {
    if (!hasWorkspaceChanges)
        return { stop: false };
    if (!isReadLikeNativeToolName(toolName))
        return { stop: false };
    if (currentChangedFileCount > lastChangedFileCount)
        return { stop: false };
    if (postPatchReadOnlySteps < 12)
        return { stop: false };
    return {
        stop: true,
        reason: [
...
export function normalizeCodexShellCommand(command) {
    ...
    if (shMatch)
        return shMatch[2].trim();
    return trimmed;
}
export function mapCodexCommandExecution(command, aggregatedOutput = '') {
...
export class CodexCliProvider {
    ...
    constructor(projectPath, model, mode = 'governed') {
    ...
    async plan(prompt, context, options) {
    ...
    async chatWithContext(messages, systemPrompt, context, tools, options) {
    ...
    shouldAllowWrites(tools) {
        if (!tools?.length)
            return true;
        return tools.some((tool) => tool.name === 'write_file' || tool.name === 'edit_file');
    }
    buildPrompt(messages, systemPrompt, context, tools) {
    ...
    isImplementationTask(taskDescription) {
        const lowered = taskDescription.toLowerCase();
        return (lowered.includes('实现')
            || lowered.includes('修改')
            || lowered.includes('新增')
    ...
    isDocumentationWorkflowTask(taskDescription) {
        if (/workspace parallel_ace subtask/i.test(taskDescription))
            return false;
    ...
    async runCodex(prompt, allowWrites, options) {
        if (!hasCodexCli()) {
    ...
    buildHarnessSessionKey(providerSessionId, allowWrites, cwd) {
        return [
            this.name,
            providerSessionId,
            this.model || 'default',
    ...
    async spawnCodex(args, beforeFiles, patchFirstRequired, options) {
                        ...
                        if (parsed) {
                            ...
                            if (!policyViolation
                                ...
                                if (this.isValidationLikeCommand(normalized) && hasWorkspaceChanges) {
                                    ...
                                    const stopDecision = shouldStopForPostPatchValidation(normalized, patchFirstRequired, hasWorkspaceChanges, validationAttemptsAfterPatch, null);
                            ...
                            if (!policyViolation
                                ...
                                const stopDecision = shouldStopForPostPatchValidation(normalized, patchFirstRequired, hasWorkspaceChanges, validationAttemptsAfterPatch, typeof parsed.item.exit_code === 'number' ? parsed.item.exit_code : null);
                                if (stopDecision.stop) {
                                    gracefulStop = stopDecision.reason;
                                    child.kill('SIGTERM');
                                    continue;
                                ...
                                if (currentChangedFileCount > lastChangedFileCount) {
                                    ...
                                    const readLoopDecision = shouldStopForPostPatchReadLoop(mapped.toolName, hasWorkspaceChanges, currentChangedFileCount, lastChangedFileCount, postPatchReadOnlySteps);
    ...
    isValidationLikeCommand(normalizedCommand) {
        return isValidationLikeCommandValue(normalizedCommand);
    }
    hasNewWorkspaceChanges(beforeFiles, cwd = this.projectPath) {
        return this.captureChangedFilesSync(cwd).size > beforeFiles.size;
    }
    captureChangedFilesSync(cwd = this.projectPath) {
    ...
    async captureChangedFiles(cwd = this.projectPath) {
        const result = spawnSync('git', ['-C', cwd, 'status', '--porcelain'], {
            encoding: 'utf-8',
        });
        if (result.status !== 0)
    ...
    async readLastMessage(lastMessagePath, stdout, stderr) {
        try {
    ...
    emitProviderRuntimeEvent(event, options) {
        const item = event.item;
        if (!item || item.type !== 'command_execution' || !item.command)
            return;
...
function isValidationLikeCommandValue(normalizedCommand) {
    return (/^mvn\b/.test(normalizedCommand)
...
function isReadLikeNativeToolName(toolName) {
...
function extractDelegateHints(context, prefixes) {
    if (!context)
        return [];
