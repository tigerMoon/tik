...
export function registerWorktreeCommands(program, services) {
    program
        .command('worktree')
        ...
        if (command === 'list' || command === 'status') {
            ...
            for (const entry of entries) {
                ...
                if (entry.worktree?.sourceBranch)
                    console.log(`    ${chalk.dim(`source-branch: ${entry.worktree.sourceBranch}`)}`);
                if (entry.worktree?.worktreeBranch)
                    console.log(`    ${chalk.dim(`worktree-branch: ${entry.worktree.worktreeBranch}`)}`);
                if (typeof entry.dirtyFileCount === 'number')
                    console.log(`    ${chalk.dim(`dirty-files: ${entry.dirtyFileCount}`)}`);
                for (const warning of entry.warnings || []) {
                    console.log(`    ${chalk.yellow(warning)}`);
                }
                if (entry.worktree?.lastError)
                    console.log(`    ${chalk.red(entry.worktree.lastError)}`);
            }
            console.log('');
            return;
        }
        ...
        const selectedEntry = services.selectManagedWorktreeEntry(entries, selectedProject.projectName, selectedProject.sourceProjectPath || selectedProject.projectPath, opts.lane);
        ...
        if (command === 'path') {
            ...
            if (selectedEntry?.laneId)
                console.log(chalk.dim(`  Lane: ${selectedEntry.laneId}`));
        ...
        if (command === 'create') {
            ...
            console.log(chalk.bold('\n🪵 Worktree Ready\n'));
            console.log(chalk.dim(`  Project: ${selectedProject.projectName}`));
            if (target.worktree?.laneId)
                console.log(chalk.dim(`  Lane:   ${target.worktree.laneId}`));
            console.log(chalk.dim(`  Source: ${target.sourceProjectPath}`));
            console.log(chalk.dim(`  Exec:   ${target.effectiveProjectPath}`));
            if (target.worktree?.worktreeBranch)
                console.log(chalk.dim(`  Branch: ${target.worktree.worktreeBranch}`));
            console.log('');
            return;
        }
        if (command === 'use') {
            ...
            const currentActiveEntry = entries.find((entry) => (entry.projectName === selectedProject.projectName
            ...
            await services.workspaceOrchestrator.activateProjectWorktreeLane(resolution.workspace.rootPath, selectedProject.projectName, {
                effectiveProjectPath: selectedEntry.effectiveProjectPath,
                worktree: selectedEntry.worktree,
            });
            console.log(chalk.bold('\n🪵 Worktree Lane Activated\n'));
            console.log(chalk.dim(`  Project: ${selectedProject.projectName}`));
            if (selectedEntry.worktree.laneId)
                console.log(chalk.dim(`  Lane:   ${selectedEntry.worktree.laneId}`));
        ...
        if (command === 'remove') {
            ...
            console.log(chalk.bold('\n🪵 Worktree Removed\n'));
            console.log(chalk.dim(`  Project: ${selectedProject.projectName}`));
            if (removed.worktree?.laneId)
                console.log(chalk.dim(`  Lane: ${removed.worktree.laneId}`));
            console.log(chalk.dim(`  Source: ${removed.sourceProjectPath}`));
            if (removed.worktree?.worktreePath)
                console.log(chalk.dim(`  Removed path: ${removed.worktree.worktreePath}`));
            if (removed.worktree?.worktreeBranch)
                console.log(chalk.dim(`  Branch retained: ${removed.worktree.worktreeBranch}`));
            console.log('');
            return;
        }
...
//# sourceMappingURL=worktree-command.js.map