...
export function resolveProvider(provider = 'codex') {
    const envProvider = process.env.TIK_LLM_PROVIDER || 'codex';
    const requested = provider === 'auto' ? envProvider : provider;
    if (requested === 'mock')
        return 'mock';
    if (requested === 'claude') {
        if (!hasClaudeCredentials())
            throw new Error('Claude credentials not found. Set ANTHROPIC_API_KEY.');
        return 'claude';
    }
    if (requested === 'openai') {
        if (!hasOpenAICredentials())
            throw new Error('OpenAI credentials not found. Set OPENAI_API_KEY.');
        return 'openai';
    }
    if (requested === 'codex') {
        if (!hasCodexCli())
            throw new Error('Codex CLI not found. Install `codex` first.');
        if (!hasCodexLogin())
