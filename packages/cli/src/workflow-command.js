...
import { loadTrackerWorkflow, } from '@tik/kernel';
export async function initWorkflowV2(options) {
...
export async function explainWorkflowTask(options) {
    const workflow = await loadWorkflowForCli(options.workspaceRoot, options.file);
...
export function defaultTrackerWorkflowV2Content() {
    return [
        '---',
        'version: 2',
        'tracker:',
...
export function formatWorkflowValidation(workflow) {
    return [
...
export function formatWorkflowExplain(workflow, task, routing) {
...
function resolveWorkflowCliPath(workspaceRoot, file) {
    const resolvedRoot = path.resolve(workspaceRoot);
    if (!file)
        return path.join(resolvedRoot, 'WORKFLOW.md');
    return path.isAbsolute(file) ? file : path.join(resolvedRoot, file);
}
function loadWorkflowForCli(workspaceRoot, file) {
...
function makeExplainTask(taskId, task) {
    return {
        id: task?.id || taskId,
...
//# sourceMappingURL=workflow-command.js.map