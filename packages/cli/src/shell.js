...
import { createCliSession, listCliSessions, loadCliSession, saveCliSession, } from './session-store.js';
export async function runShell({ config, createRuntime }) {
...
async function handleShellCommand(command, session, state) {
    switch (command.type) {
        case 'help':
            console.log('');
    ...
    switch (name) {
        ...
        case 'session':
            if (parts[1] === 'list')
                return { type: 'sessions' };
            if (parts[1] === 'switch')
                return { type: 'resume', target: parts[2] };
...
function attachEventStream(kernel, onEvent) {
    return kernel.eventBus.onAny((event) => {
        onEvent?.(event);
        displayEvent(event);
    });
}
function createRuntimeForSession(createRuntime, session) {
...
function printShellHeader(session, resolution, resumed) {
...
async function promptOnce(promptText) {
...
function pathLabel(projectPath) {
    const parts = projectPath.split('/').filter(Boolean);
    return parts[parts.length - 1] || projectPath;
}
async function renderMemorySummary(session) {
    const fs = await import('node:fs/promises');
    const path = await import('node:path');
    ...
    try {
        const failures = JSON.parse(await fs.readFile(failuresPath, 'utf-8'));
    ...
    try {
        const decisions = JSON.parse(await fs.readFile(decisionsPath, 'utf-8'));
...
async function renderDiffSummary(projectPath) {
...
function renderConfigSummary(session, section) {
    const shellLines = [
        `  Session:       ${session.sessionId}`,
        `  Project:       ${session.projectPath}`,
        `  Provider:      ${session.provider}`,
...
async function initializeProjectInstructions(projectPath, force) {
    const fs = await import('node:fs/promises');
    const path = await import('node:path');
    const targets = [
        {
