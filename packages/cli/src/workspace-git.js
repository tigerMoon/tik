...
export async function captureWorkspaceGitChangedFiles(projectPath) {
...
export function listGitWorktrees(projectPath) {
    ...
    for (const rawLine of result.stdout.split('\n')) {
        ...
        if (!line) {
            if (current.path)
                entries.push({ path: current.path, branch: current.branch, bare: current.bare });
            current = {};
            continue;
        }
