...
import { buildDelegateDocumentationContract, buildDelegateImplementationContract, mapCodexCommandExecution, normalizeCodexShellCommand, shouldStopForPostPatchReadLoop, shouldStopForPostPatchValidation, } from '../src/commands/codex-cli.js';
describe('codex native command mapping', () => {
    ...
    it('stops after a failed first validation attempt once a patch exists', () => {
        const decision = shouldStopForPostPatchValidation('mvn -pl catalog-suite-application test', true, true, 1, 1);
    ...
    it('stops before a repeated validation attempt once a patch exists', () => {
        const decision = shouldStopForPostPatchValidation('mvn -pl catalog-suite-application test', true, true, 2, null);
        expect(decision.stop).toBe(true);
    ...
    it('does not stop non-validation commands after a patch exists', () => {
        const decision = shouldStopForPostPatchValidation('git diff -- catalog-suite-application/src/main/java', true, true, 0, 0);
        expect(decision.stop).toBe(false);
    });
    it('stops after too many post-patch read-only steps without new writes', () => {
        const decision = shouldStopForPostPatchReadLoop('read_file', true, 2, 2, 12);
        expect(decision.stop).toBe(true);
    ...
    it('does not stop post-patch read loop when a new file changed', () => {
        const decision = shouldStopForPostPatchReadLoop('read_file', true, 3, 2, 20);
        expect(decision.stop).toBe(false);
    });
    it('builds a delegate implementation contract for autonomous subtask completion', () => {
        const contract = buildDelegateImplementationContract('我想针对票务业务的查询接口做缓存，one-api目录', [
        ...
        ].join('\n'), 'Implementation mode is active.', true);
    ...
    it('keeps delegate contract concise when no context hints are available', () => {
        const contract = buildDelegateImplementationContract('implement caching', '', '', false);
