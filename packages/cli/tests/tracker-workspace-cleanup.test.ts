...
import {
  cleanupManagedTrackerWorkspace,
  isManagedTrackerWorktreePath,
} from '../src/tracker-workspace-cleanup.js';
...
describe('tracker workspace cleanup safety', () => {
  it('only treats direct children under the workspace worktrees root as managed worktrees', () => {
    ...
    expect(isManagedTrackerWorktreePath(
      workspaceRoot,
      path.join(workspaceRoot, '.workspace', 'worktrees', 'service-a--tik-1'),
    )).toBe(true);

    expect(isManagedTrackerWorktreePath(
      workspaceRoot,
      path.join(workspaceRoot, '.workspace', 'worktrees'),
    )).toBe(false);

    expect(isManagedTrackerWorktreePath(
      workspaceRoot,
      path.join(workspaceRoot, '.workspace', 'worktrees', '..', '..', '..', 'victim'),
    )).toBe(false);

    expect(isManagedTrackerWorktreePath(
      workspaceRoot,
      path.join(workspaceRoot, '.workspace', 'worktrees-evil', 'service-a'),
    )).toBe(false);
  });
