...
import { buildWorkspaceFeatureDir, buildWorkspaceFeatureName, buildWorkspacePlanTargetPath, buildWorkspaceSpecTargetPath, resolveWorkspacePlanArtifact, resolveWorkspaceSpecArtifact, workspaceFeatureDirForArtifact, } from '../src/workspace-artifacts.js';
const tempDirs = [];
afterEach(async () => {
...
describe('workspace artifact resolution', () => {
    it('builds deterministic workspace feature target paths', async () => {
        ...
        expect(buildWorkspaceSpecTargetPath(projectPath, 'require-category', '给 service-b 增加缓存并同步 service-a 契约')).toBe(path.join(featureDir, 'spec.md'));
        expect(buildWorkspacePlanTargetPath(projectPath, 'require-category', '给 service-b 增加缓存并同步 service-a 契约')).toBe(path.join(featureDir, 'plan.md'));
    });
...
//# sourceMappingURL=workspace-artifacts.test.js.map