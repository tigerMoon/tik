...
describe('artifacts command helpers', () => {
  it('formats artifact lists with review metadata', () => {
    ...
    ] as never)).toContain('art_abc123  needs_review  3    report    TIK-42');
  });
  ...
  it('builds artifact-id preview URLs instead of path preview URLs', () => {
    expect(buildArtifactPreviewApiUrl('http://localhost:3000/', 'art_abc123', 'ver_123')).toBe(
      'http://localhost:3000/api/workbench/artifacts/art_abc123/versions/ver_123/preview',
    );
  });
});
